	...
	private function build_page()
	{
		...
		$count				= parent::$db->query_fetch ( "SELECT
															(SELECT COUNT(fleet_owner) AS `actcnt`
																FROM " . FLEETS . "
																WHERE `fleet_owner` = '" . intval ( $this->_current_user['user_id'] ) . "') AS max_fleet,
		...
		$galaxy         				= isset ( $_GET['galaxy'] ) ? intval ( $_GET['galaxy'] ) : $this->_current_planet['galaxy'];
		$system         				= isset ( $_GET['system'] ) ? intval ( $_GET['system'] ) : $this->_current_planet['system'];
		$planet         				= isset ( $_GET['planet'] ) ? intval ( $_GET['planet'] ) : $this->_current_planet['planet'];
		$planettype     				= isset ( $_GET['planettype'] ) ? intval ( $_GET['planettype'] ) : $this->_current_planet['planet_type'];
		$target_mission 				= isset ( $_GET['target_mission'] ) ? intval ( $_GET['target_mission'] ) : FALSE;
