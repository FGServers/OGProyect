...
class Defense extends OGPCore
{
	...
	public function build_page()
	{
		...
		if ( isset ( $_POST['fmenge'] ) )
		{
			...
			$BuildQueue    	= $this->_current_planet['b_hangar_id'];
			...
			foreach($_POST['fmenge'] as $Element => $Count)
			{
				...
				if ($Count != 0)
				{
					...
					if (Developments_Lib::is_development_allowed($this->_current_user, $this->_current_planet, $Element))
					{
						...
						if ($Count >= 1)
						{
							$this->_current_planet['metal']           -= $Ressource['metal'];
							$this->_current_planet['crystal']         -= $Ressource['crystal'];
							$this->_current_planet['deuterium']       -= $Ressource['deuterium'];
							$this->_current_planet['b_hangar_id']     .= "". $Element .",". $Count .";";
						}
					}
				}
			}
			...
			if ( $totalCount > 0 )
			{
				parent::$db->query ( "UPDATE " . PLANETS . " AS p SET
										p.`b_hangar_id` = '".$this->_current_planet['b_hangar_id']."',
										p.`metal` = '".$this->_current_planet['metal']."',
										p.`crystal` = '".$this->_current_planet['crystal']."',
										p.`deuterium` = '".$this->_current_planet['deuterium']."'
		...
		foreach($this->_lang['tech'] as $Element => $ElementName)
		{
			if ($Element > 400 && $Element <= 599)
			{
				if (Developments_Lib::is_development_allowed($this->_current_user, $this->_current_planet, $Element))
				{
					...
					{
						$InQueue = strpos ( $this->_current_planet['b_hangar_id'], $Element.",");
		...
		if ($this->_current_planet['b_hangar_id'] != '')
	...
	private function GetMaxConstructibleElements ( $Element , $Ressources )
    {
        ...
        if ($this->_price[$Element]['energy'] != 0)
            $Buildable['energy']    = floor($Ressources["energy_max"] / $this->_price[$Element]['energy']);
	...
	{
		$ElementQueue 	= explode ( ';' , $this->_current_planet['b_hangar_id'] );
		$NbrePerType  	= "";
		$NamePerType  	= "";
		$TimePerType	= "";
		$QueueTime		= 0;
		...
		$parse['b'] 					= $NamePerType;
		$parse['c'] 					= $TimePerType;
		$parse['b_hangar_id_plus'] 		= $this->_current_planet['b_hangar'];
		$parse['current_page']			= $current_page;
		$parse['pretty_time_b_hangar'] 	= Format_Lib::pretty_time($QueueTime - $this->_current_planet['b_hangar']);
