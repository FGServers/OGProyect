...
class Buildings extends OGPCore
{
	const MODULE_ID	= 3;
	...
	private function build_page()
	{
		...
		foreach ( $this->_lang['tech'] as $building => $building_name )
		{
			if ( in_array ( $building , $this->_allowed[$this->_current_planet['planet_type']] ) )
			{
				...
				if ( $this->_current_planet['field_current'] < ( $max_fields - $queue['to_destroy'] ) )
				{
					$have_fields = TRUE;
				}
				else
	...
	private function can_build ( $building , $list_id )
	{
		...
		if ( ( $building == 21 or $building == 14 or $building == 15 ) && $this->_current_planet['b_hangar'] != 0 )
		{
			$continue 	= FALSE;
		}
	...
	private function cancel_current()
	{
		...
		if ( $CurrentQueue != 0 )
		{
			...
			if ( $building != FALSE )
			{
				...
				$this->_current_planet['metal']       += $Needed['metal'];
				$this->_current_planet['crystal']     += $Needed['crystal'];
				$this->_current_planet['deuterium']   += $Needed['deuterium'];
			}
	...
	private function add_to_queue ( $building , $AddMode = TRUE )
	{
		...
		if ( ( $this->_current_planet['field_current'] >= ( $max_fields - $queue['lenght'] ) ) )
		{
			Functions_Lib::redirect ( 'game.php?page=' . $this->_current_page );
		}
