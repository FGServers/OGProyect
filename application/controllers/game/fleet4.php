...
class Fleet4 extends OGPCore
{
	...
	private function build_page()
	{
		...
		$TargetPlanet  		= parent::$db->query_fetch ( "SELECT `id_owner`,`planet_destroyed`
															FROM " . PLANETS . "
															 WHERE `galaxy` = '". intval($_POST['galaxy']) ."' AND
															 		`system` = '". intval($_POST['system']) ."' AND
															 		`planet` = '". intval($_POST['planet']) ."' AND
															 		`planet_type` = '". intval($_POST['planettype']) ."';");
		...
		if ($fleetmission == 8)
		{
			...
			if($select['planet_debris_metal'] == 0 && $select['planet_debris_crystal'] == 0 && time() > ($select['invisible_start_time']+DEBRIS_LIFE_TIME))
			{
				Functions_Lib::redirect ( 'game.php?page=movement' );
			}
		}
		...
		if ($this->_current_planet['galaxy'] == $galaxy && $this->_current_planet['system'] == $system &&
			$this->_current_planet['planet'] == $planet && $this->_current_planet['planet_type'] == $planettype)
		{
			Functions_Lib::redirect ( 'game.php?page=movement' );
		}
		...
		if ($select['id_owner'] == $this->_current_user['user_id'])
		{
			$YourPlanet = TRUE;
			$UsedPlanet = TRUE;
		}
		...
		elseif ( $TargetPlanet['id_owner'] != '' )
		{
			$HeDBRec 	= parent::$db->query_fetch ( "SELECT u.`user_id`, u.`user_onlinetime`, u.`user_ally_id`, s.`setting_vacations_status`
														FROM " . USERS . " AS u, " . SETTINGS . " AS s
														WHERE u.`user_id` = '" . $TargetPlanet['id_owner'] ."'
															AND s.`setting_user_id` ='" . $TargetPlanet['id_owner'] ."';" );
		}
		...
		if ($_POST['mission'] != 15)
		{
			...
			if (($TargetPlanet["id_owner"] == $this->_current_planet["id_owner"]) && (($_POST["mission"] == 1) or ($_POST["mission"] == 6)))
			{
				Functions_Lib::redirect ( 'game.php?page=movement' );
			}

			if (($TargetPlanet["id_owner"] != $this->_current_planet["id_owner"]) && ($_POST["mission"] == 4))
			{
			...
			if($_POST['mission'] == 5)
			{
				$buddy = parent::$db->query_fetch ( "SELECT COUNT( * ) AS buddys
														FROM  `" . BUDDY . "`
															WHERE (
																(
																	buddy_sender ='" . intval($this->_current_planet['id_owner']) . "'
																	AND buddy_receiver ='" . intval($TargetPlanet['id_owner']) . "'
																)
																OR (
																	buddy_sender ='" . intval($TargetPlanet['id_owner']) . "'
																	AND buddy_receiver ='" . intval($this->_current_planet['id_owner']) . "'
																)
															)
															AND buddy_status =1" );
		...
		if ($_POST['thisgalaxy'] != $this->_current_planet['galaxy'] |
			$_POST['thissystem'] != $this->_current_planet['system'] |
			$_POST['thisplanet'] != $this->_current_planet['planet'] |
		...
		$StockMetal      = $this->_current_planet['metal'];
		$StockCrystal    = $this->_current_planet['crystal'];
		$StockDeuterium  = $this->_current_planet['deuterium'];
		...
		parent::$db->query ( "UPDATE `" . PLANETS . "` AS p
								...
								$FleetSubQRY
								`metal` = `metal` - ". $TransMetal .",
								`crystal` = `crystal` - ". $TransCrystal .",
								`deuterium` = `deuterium` - ". ($TransDeuterium + $consumption) ."
