...
class Alliance extends OGPCore
{
	const MODULE_ID	= 13;
	...
	public function __construct()
	{
		...
		$this->_lang['img_path']	= OGP_ROOT . IMG_PATH;
		$this->_lang['js_path']		= OGP_ROOT . JS_PATH;
	...
	{
		// GET ALLY ID
		$ally_id	= isset ( $_GET['allyid'] ) ? intval ( $_GET['allyid'] ) : NULL;
		$id 		= isset ( $_GET['id'] ) ? intval ( $_GET['id'] ) : NULL;

		// VALIDATE AND GET ALLIANCE DATA
		if ( is_numeric ( $ally_id ) && $ally_id != 0 && intval ( $ally_id ) )
		{
		...
		return parent::$page->parse_template	(
													...
													$this->_lang +
													array 	(
																'ally_image' 			=> $this->image_block ( $ally_image ),
																'ally_tag' 				=> $ally_tag,
																'alliance_name' 		=> $alliance_name,
																'ally_member_scount' 	=> $ally_members,
																'ally_description' 		=> $this->description_block ( $ally_description ),
																'alliance_web' 			=> $this->web_block ( $alliance_web ),
																'ally_request'			=> $this->request_block ( $id , $ally_request_notallow )
															)
												);
	}
	...
	private function ally_make()
	{
		if ( $this->_current_user['user_ally_id'] == 0 && $this->_current_user['user_ally_request'] == 0 )
		{
			...
			{
				$ally_tag		= $this->check_tag ( $_POST['atag'] );
				$alliance_name	= $this->check_name ( $_POST['aname'] );

				parent::$db->query ( "INSERT INTO " . ALLIANCE . " SET
										`alliance_name`='" . $alliance_name . "',
										`ally_tag`='" . $ally_tag . "' ,
										`ally_owner`='" . intval ( $this->_current_user['user_id'] ) . "',
										`ally_owner_range` = '" . $this->_lang['al_alliance_founder_rank'] . "',
				...
				parent::$db->query ( "UPDATE " . USERS . " SET
										...
										WHERE `user_id`='" . intval ( $this->_current_user['user_id'] ) . "'" );

				$message	= str_replace ( array ( '%s' , '%d' ) , array ( $alliance_name , $ally_tag ) , $this->_lang['al_created'] );
			...
			if ( $_POST )
			{
				$search = parent::$db->query ( "SELECT a.*,
												...
												WHERE a.alliance_name LIKE '%" . parent::$db->escape_value ( $_POST['searchtext'] ) . "%' OR
														a.ally_tag LIKE '%" . parent::$db->escape_value ( $_POST['searchtext'] ) . "%' LIMIT 30" );
		...
		if ( $this->_current_user['user_ally_id'] == 0 && $this->_current_user['user_ally_request'] == 0 )
		{
			if ( $_GET['allyid'] != NULL )
			{
				$alianza = parent::$db->query_fetch ( "SELECT *
														FROM " . ALLIANCE . "
														WHERE alliance_id = '" . intval ( $_GET['allyid'] ) . "'" );
			}
			...
			else
			{
				...
				$allyrow = parent::$db->query_fetch ( "SELECT ally_tag, ally_request
														FROM " . ALLIANCE . "
														WHERE alliance_id = '" . intval ( $_GET['allyid'] ) . "'" );
	...
	private function ally_main()
	{
		...
		if ( $this->_current_user['user_ally_id'] == 0 && $this->_current_user['user_ally_request'] != 0 )
		{
			$allyquery = parent::$db->query_fetch ( "SELECT `ally_tag`
														FROM " . ALLIANCE . "
														WHERE alliance_id = '" . intval ( $this->_current_user['user_ally_request'] ) . "' ORDER BY `alliance_id`" );
			...
			if ( isset ( $_POST['bcancel'] ) )
			{
				...
				$this->_lang['request_text']	= str_replace ( '%s' , $ally_tag , $this->_lang['al_request_deleted'] );
				$this->_lang['button_text'] 	= $this->_lang['al_continue'];
			}
			else
			{
				$this->_lang['request_text']	= str_replace ( '%s' , $ally_tag , $this->_lang['al_request_wait_message'] );
		...
		{
			// IMAGE
			if ( $this->_ally['ally_ranks'] != '' )
			{
				$parse['ally_image']			=	$this->image_block ( $this->_ally['ally_image'] );
				$this->_ally['ally_ranks']	 	=	parent::$page->parse_template ( parent::$page->get_template ( 'alliance/alliance_image_row' ) ,  $parse );
			}

			// RANKS
			if ( $this->_ally['ally_owner'] == $this->_current_user['user_id'] )
			{
				$range	= ( $this->_ally['ally_owner_range'] != '' ) ? $this->_ally['ally_owner_range'] : $this->_lang['al_founder_rank_text'];
			}
			elseif ( $this->_current_user['user_ally_rank_id'] != 0 && isset ( $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['name'] ) )
			{
				$range	= $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['name'];
			}
			else
			{
			...
			// MEMBER LIST
			if ( $this->_ally['ally_owner'] == $this->_current_user['user_id'] or $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['memberlist'] != 0 )
			{
			...
			// ADMIN ALLIANCE
			if ($this->_ally['ally_owner'] == $this->_current_user['user_id'] or $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['administrieren'] != 0)
			{
			...
			// CIRCULAR MESSAGE
			if ($this->_ally['ally_owner'] == $this->_current_user['user_id'] or $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['mails'] != 0)
			{
			...
			$request_count	= parent::$db->num_rows ( parent::$db->query ( "SELECT `user_id`
																				FROM `" . USERS . "`
																				WHERE `user_ally_request` = '" . intval ( $this->_ally['alliance_id'] ) . "'" ) );

			if ( $request_count != 0 )
			{
				if ( $this->_ally['ally_owner'] == $this->_current_user['user_id'] or $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['bewerbungen'] != 0 )
				{
			...
			if ( $this->_ally['ally_owner'] != $this->_current_user['user_id'] )
			{
				$this->_lang['ally_owner']			= parent::$page->parse_template ( parent::$page->get_template ( 'alliance/alliance_abandon_alliance' ) , $parse );
			}
			else
			{
				$this->_lang['ally_owner']			= '';
			}

			// GENERAL INFORMATION
			$this->_lang['ally_image'] 				= ( $this->_ally['ally_image'] != '' ) ? "<tr><th colspan=2><img src=\"{$this->_ally['ally_image']}\"></td></tr>" : '';
			$this->_lang['range'] 					= $range;
			$this->_lang['ally_description'] 		= $this->description_block ( $this->_ally['ally_description'] );
			$this->_lang['ally_text'] 				= nl2br ( $this->_bbcode->bbCode ( $this->_ally['ally_text'] ) );
			$this->_lang['alliance_web']			= $this->web_block ( $this->_ally['alliance_web'] );
			$this->_lang['ally_tag'] 				= $this->_ally['ally_tag'];
	...
	private function ally_members_list()
	{
		...
		if ( $this->_current_user['user_ally_id'] != 0 && $this->_current_user['user_ally_request'] == 0 && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['see_users_list'] ) === TRUE )
		{
			$sort1 			= isset ( $_GET['sort1'] ) ? intval ( $_GET['sort1'] ) : NULL; // ORDEN 1
			$sort2 			= isset ( $_GET['sort2'] ) ? intval ( $_GET['sort2'] ) : NULL; // ORDEN 2
			...
			while ( $u = parent::$db->fetch_array ( $listuser ) )
			{
				...
				if ( $this->_ally['ally_owner'] == $u['user_id'] )
				{
					$u["user_ally_range"] 	= ( $this->_ally['ally_owner_range'] == '' ) ? $this->_lang['al_founder_rank_text'] : $this->_ally['ally_owner_range'];
				...
				{
					$u["user_ally_range"] 	= $ally_ranks[$u['user_ally_rank_id']-1]['name'];
				}
	...
	private function ally_circular_message()
	{
		...
		if ( $this->_current_user['user_ally_id'] != 0 && $this->_current_user['user_ally_request'] == 0 && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['create_circular'] ) === TRUE )
		{
			if ( isset ( $_GET['sendmail'] ) && $_GET['sendmail'] == 1 )
			{
				...
				{
					Functions_Lib::send_message ( $u['user_id'] , $this->_current_user['user_id'] , '' , 3 , $this->_ally['ally_tag'] , $this->_current_user['user_name'] , $_POST['text'] );
			...
			$ally_ranks	= unserialize ( $this->_ally['ally_ranks'] );
	...
	private function ally_admin()
	{
		...
		if ( $this->_current_user['user_ally_id'] != 0 && $this->_current_user['user_ally_request'] == 0 )
		{
			...
			switch ( $edit )
			{
				case ( $edit == 'rights' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['right_hand'] ) === TRUE ):
					...
					elseif ( isset ( $_POST['id'] ) && $_POST['id'] != '' && is_array ( $_POST['id'] ) )
					{
						...
						foreach ( $_POST['id'] as $id )
						{
							...
							$ally_ranks_new[$id]['kick']					= ( isset ( $_POST['u' . $id . 'r1'] ) && $this->_ally['ally_owner'] == $this->_current_user['user_id'] ) ? 1 : 0;
					...
					elseif ( isset ( $d ) && isset ( $ally_ranks[$d] ) )
					{
						...
						$this->_ally['ally_rank']	= serialize ( $ally_ranks );

						parent::$db->query ( "UPDATE " . ALLIANCE . " SET
												`ally_ranks`='" . $this->_ally['ally_rank'] . "'
												WHERE `alliance_id` = " . $this->_ally['alliance_id'] . "" );
					}
				...
				default:
					...
					{
						$this->_ally['ally_owner_range'] 		= parent::$db->escape_value ( htmlspecialchars ( strip_tags ( $_POST['owner_range'] ) ) );
						$this->_ally['alliance_web'] 			= parent::$db->escape_value ( htmlspecialchars ( strip_tags ( $_POST['web'] ) ) );
						$this->_ally['ally_image'] 				= parent::$db->escape_value ( htmlspecialchars ( strip_tags ( $_POST['image'] ) ) );
						$this->_ally['ally_request_notallow']	= intval ( $_POST['request_notallow'] );

						if ( $this->_ally['ally_request_notallow'] != 0 && $this->_ally['ally_request_notallow'] != 1 )
						{
							Functions_Lib::redirect ( 'game.php?page=alliance?mode=admin&edit=ally' );
						}

						parent::$db->query ( "UPDATE " . ALLIANCE . " SET
												`ally_owner_range`='" . $this->_ally['ally_owner_range'] . "',
												`ally_image`='" . $this->_ally['ally_image'] . "',
												`alliance_web`='" . $this->_ally['alliance_web'] . "',
												`ally_request_notallow`='" . $this->_ally['ally_request_notallow'] . "'
												WHERE `alliance_id` = '" . $this->_ally['alliance_id'] . "'" );
					}
					elseif ( isset ( $_POST['t'] ) )
					{
						if ( $t == 3 )
						{
							$this->_ally['ally_request']		= $_POST['text'];

							parent::$db->query ( "UPDATE " . ALLIANCE . " SET
													`ally_request`='" . $this->_ally['ally_request'] . "'
													WHERE `alliance_id` = '" . $this->_ally['alliance_id'] . "'" );
						...
						elseif ($t == 2)
						{
							$this->_ally['ally_text']			= $_POST['text'];

							parent::$db->query ( "UPDATE " . ALLIANCE . " SET
													`ally_text`='" . $this->_ally['ally_text'] . "'
													WHERE `alliance_id` = '" . $this->_ally['alliance_id'] . "'" );
						...
						{
							$this->_ally['ally_description']	= $_POST['text'];

							parent::$db->query ( "UPDATE " . ALLIANCE . " SET
													`ally_description`='" . $this->_ally['ally_description'] . "'
													WHERE `alliance_id` = '" . $this->_ally['alliance_id'] . "'" );
					...
					if ( $t == 2 )
					{
						$this->_lang['text'] 					= $this->_ally['ally_text'];
					}
					else
					{
						$this->_lang['text'] 					= $this->_ally['ally_description'];
					}

					if ( $t == 3 )
					{
						$this->_lang['text'] 					= $this->_ally['ally_request'];
					}

					$this->_lang['t'] 							= $t;
					$this->_lang['alliance_web'] 				= $this->_ally['alliance_web'];
					$this->_lang['ally_image'] 					= $this->_ally['ally_image'];
					$this->_lang['ally_request_notallow_0'] 	= (($this->_ally['ally_request_notallow'] == 1) ? ' SELECTED' : '');
					$this->_lang['ally_request_notallow_1'] 	= (($this->_ally['ally_request_notallow'] == 0) ? ' SELECTED' : '');
					$this->_lang['ally_owner_range'] 			= $this->_ally['ally_owner_range'];

					return parent::$page->parse_template ( parent::$page->get_template ( 'alliance/alliance_admin' ) , $this->_lang );

				break;

				case ( $edit == 'members' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['admin_alliance'] ) === TRUE ):

					$rank	= isset ( $_GET['rank'] ) ? intval ( $_GET['rank'] ) : NULL;
					$kick 	= isset ( $_GET['kick'] ) ? intval ( $_GET['kick'] ) : NULL;
					$id 	= isset ( $_GET['id'] ) ? intval ( $_GET['id'] ) : NULL;
					...
					if ( isset ( $kick ) )
					{
						$this->have_access ( $this->_ally['ally_owner'] , $this->permissions['kick_users'] );
						...
						if ( $u['user_ally_id'] == $this->_ally['alliance_id'] && $u['user_id'] != $this->_ally['ally_owner'] )
						{
							parent::$db->query ( "UPDATE " . USERS . " SET
													`user_ally_id`='0',
													`user_ally_rank_id` = 0
													WHERE `user_id`='" . intval ( $u['id'] ) . "' LIMIT 1;" );
						}
					}
					elseif ( isset ( $_POST['newrang'] ) )
					{
						...
						if ( ( isset ( $ally_ranks[$_POST['newrang']-1] ) or $_POST['newrang'] == 0 ) && ( $q['user_id'] != $this->_ally['ally_owner'] ) )
						{
							parent::$db->query ( "UPDATE " . USERS . " SET
					...
					while ( $u = parent::$db->fetch_array ( $listuser ) )
					{
						...
						if ( $this->_ally['ally_owner'] == $u['user_id'] )
						{
							$ally_range 		= ( $this->_ally['ally_owner_range'] == '' ) ? $this->_lang['al_founder_rank_text'] : $this->_ally['ally_owner_range'];
						}
						elseif ( $u['user_ally_rank_id'] == 0 or !isset ( $ally_ranks[$u['user_ally_rank_id']-1]['name'] ) )
						{
							$ally_range 		= $this->_lang['al_new_member_rank_text'];
						}
						else
						{
							$ally_range 		= $ally_ranks[$u['user_ally_rank_id']-1]['name'];
						}

						if ( $this->_ally['ally_owner'] == $u['user_id'] or $rank == $u['user_id'] )
						{
							$u["acciones"] 	= '-';
						}
						elseif ( $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['kick'] == 1  &&  $ally_ranks[$this->_current_user['user_ally_rank_id']-1]['administrieren'] == 1 or $this->_ally['ally_owner'] == $this->_current_user['user_id'] )
						{
						...
						elseif ($ally_ranks[$this->_current_user['user_ally_rank_id']-1]['administrieren'] == 1 )
						{
							$u["acciones"] 	= "<a href=\"game.php?page=alliance&mode=admin&edit=members&kick=" . $u['user_id'] . "\" onclick=\"javascript:return confirm('" . str_replace ( '%s' , $u['user_name'] , $this->_lang['al_confirm_remove_member'] ) . "');\"><img src=\"" . DPATH . "alliance/abort.gif\" border=\"0\"></a> <a href=\"game.php?page=alliance&mode=admin&edit=members&rank=" . $u['user_id'] . "\"><img src=\"" . DPATH . "alliance/key.gif\" border=\"0\"></a>";
						}
						else
						...
						$u["dpath"]						= DPATH;
						$u['ally_register_time']		= date ( Functions_Lib::read_config ( 'date_format_extended' ) , $u['user_ally_register_time'] );
				...
				case ( $edit == 'requests' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['check_requests'] ) === TRUE ):
					...
					if ( isset ( $_POST['action'] ) && ( $_POST['action'] == $this->_lang['al_acept_request'] ) )
					{
						...
						Functions_Lib::send_message ( $show , $this->_current_user['user_id'] , '' , 3 ,$this->_ally['ally_tag'] , $this->_lang['al_you_was_acceted'] . $this->_ally['alliance_name'] , $this->_lang['al_hi_the_alliance'] . $this->_ally['alliance_name'] . $this->_lang['al_has_accepted'] . $_POST['text'] );
					...
					elseif ( isset ( $_POST['action'] ) && ( $_POST['action'] == $this->_lang['al_decline_request'] ) && $_POST['action'] != '')
					{
						...
						Functions_Lib::send_message ( $show , $this->_current_user['user_id'] , '' , 3 , $this->_ally['ally_tag'] , $this->_lang['al_you_was_declined'] . $this->_ally['alliance_name'] , $this->_lang['al_hi_the_alliance'] . $this->_ally['alliance_name'] . $this->_lang['al_has_declined'] . $_POST['text'] );
					...
					while ( $r = parent::$db->fetch_array ( $query ) )
					{
						...
						$r['time']		= date ( Functions_Lib::read_config ( 'date_format_extended' ) , $r['ally_register_time'] );
					...
					$parse['ally_tag'] 					= $this->_ally['ally_tag'];
				...
				case ( $edit == 'name' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['admin_alliance'] ) === TRUE ):
				...
				case ( $edit == 'tag' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['admin_alliance'] ) === TRUE ):

					$ally_tag		= '';

					if ( $_POST )
					{
						$ally_tag	= $this->check_tag ( $_POST['nametag'] );
					...
					$parse['caso'] 			= ( $ally_tag == '' ) ? str_replace ( '%s' , $this->_ally['ally_tag'] , $this->_lang['al_change_title'] ) : str_replace ( '%s' , $ally_tag , $this->_lang['al_change_title'] );
				...
				case ( $edit == 'exit' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['disolve_alliance'] ) === TRUE ):
				...
				case ( $edit == 'transfer' && $this->have_access ( $this->_ally['ally_owner'] , $this->permissions['admin_alliance'] ) === TRUE ):

					if ( isset ( $_POST['newleader'] ) )
					{
						parent::$db->query ( "UPDATE " . USERS . " AS u1, " . ALLIANCE . " AS a, " . USERS . " AS u2 SET
												u1.`user_ally_rank_id` = '0',
												a.`ally_owner` = '" . parent::$db->escape_value ( strip_tags ( $_POST['newleader'] ) ) . "',
					...
					else
					{
						...
						while ( $u = parent::$db->fetch_array ( $listuser ) )
						{
							if ( $this->_ally['ally_owner'] != $u['user_id'] )
							{
								if ( $u['ally_rank_id'] != 0 )
								{
									if ( $ally_ranks[$u['user_ally_rank_id']-1]['rechtehand'] == 1 )
									{
										$righthand['righthand'] .= "\n<option value=\"" . $u['user_id'] . "\"";
										$righthand['righthand'] .= ">";
										$righthand['righthand'] .= "".$u['user_name'];
										$righthand['righthand'] .= "&nbsp;[".$ally_ranks[$u['user_ally_rank_id']-1]['name'];
	 ...
	 */
	private function image_block ( $ally_image )
	{
		if ( $ally_image != '' )
		{
			return '<tr><th colspan="2">' . Functions_Lib::set_image ( $ally_image , $ally_image ) . '</td></tr>';
		}
	...
	/**
	 * method image_block
	 * param $ally_description
	 * return shows the description block, if any
	 */
	private function description_block ( $ally_description )
	{
		if ( $ally_description == '' )
		{
			$ally_description	= $this->_lang['al_description_message'];
		}
		else
		{
			$ally_description	= nl2br ( $this->_bbcode->bbCode ( $ally_description ) ) . '</th></tr>';
		}

		return '<tr><th colspan="2" height="100px">' . $ally_description . '</th></tr>';
	}
	 ...
	 */
	private function request_block ( $ally_id , $ally_request )
	{
		if ( ! $this->_current_user['user_ally_id'] && ! $this->_current_user['user_ally_request']  && ! $ally_request )
		{
	...
	private function return_rank ( $rank_type )
	{
		...
		return ( ( isset ( $ally_ranks[$this->_current_user['user_ally_rank_id']-1][$rank_type] ) && $ally_ranks[$this->_current_user['user_ally_rank_id']-1][$rank_type] == 1 ) or $this->_ally['ally_owner'] == $this->_current_user['user_id'] );
	}
	 ...
	 */
	private function have_access ( $ally_owner , $permission )
	{
		if ( $ally_owner != $this->_current_user['user_id'] && ! $permission )
		{
			Functions_Lib::redirect ( 'game.php?page=alliance' );
		}
		else
	...
	private function check_tag ( $ally_tag )
	{
		...
		if ( $check_tag )
		{
			Functions_Lib::message ( str_replace ( '%s' , $ally_tag , parent::$this->_lang['al_tag_already_exists'] ) , "game.php?page=alliance&mode=make" , 2 );
			exit;
		}
