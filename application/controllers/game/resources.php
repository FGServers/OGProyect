...
class Resources extends OGPCore
{
	const MODULE_ID	= 4;
	...
	private function build_page()
	{
		...
		$this->_current_planet['metal_max']			 = Production_Lib::max_storable ( $this->_current_planet[ $this->_resource[22] ]);
		$this->_current_planet['crystal_max']		 = Production_Lib::max_storable ( $this->_current_planet[ $this->_resource[23] ]);
		$this->_current_planet['deuterium_max']	     = Production_Lib::max_storable ( $this->_current_planet[ $this->_resource[24] ]);

		$parse['production_level'] 			 = 100;
		$post_porcent 						 = Production_Lib::max_production ( $this->_current_planet['energy_max'] , $this->_current_planet['energy_used'] );

		$parse['resource_row']               = "";
		$this->_current_planet['metal_perhour']      = 0;
		$this->_current_planet['crystal_perhour']    = 0;
		$this->_current_planet['deuterium_perhour']  = 0;
		$this->_current_planet['energy_max']         = 0;
		$this->_current_planet['energy_used']        = 0;
		...
		foreach($this->_reslist['prod'] as $ProdID)
		{
			...
			{
				$BuildLevelFactor	= $this->_current_planet[ $this->_resource[$ProdID]."_porcent" ];
				...
				if ( $energy > 0 )
				{
					$this->_current_planet['energy_max']    += $energy;
				}
				else
				{
					$this->_current_planet['energy_used']   += $energy;
				}

				$this->_current_planet['metal_perhour']     += $metal;
				$this->_current_planet['crystal_perhour']   += $crystal;
				$this->_current_planet['deuterium_perhour'] += $deuterium;

				$geologe_bonus 	= "";
				$engineer_bonus = "";

				if ( Officiers_Lib::is_officier_active ( $this->_current_user['premium_officier_geologist'] ) > 0 )
				{
					$geologe_bonus .= "<img border=\"0\" src=\"".DPATH."premium/geologe_ikon.gif\" onmouseover='return overlib(\"<font color=white>".$this->_lang['rs_geologist_bonus']."</font>\", WIDTH, 150);' onmouseout='return nd();' width=\"20\" height=\"20\">";
				}
				else
				{
					$geologe_bonus .= "";
				...
				if ( Officiers_Lib::is_officier_active ( $this->_current_user['premium_officier_engineer'] ) > 0 )
				{
					$engineer_bonus = "<img border=\"0\" src=\"".DPATH."premium/ingenieur_ikon.gif\" onmouseover='return overlib(\"<font color=white>".$this->_lang['rs_engineer_bonus']."</font>\", WIDTH, 150);' onmouseout='return nd();' width=\"20\" height=\"20\">";
				}
				else
				{
					$engineer_bonus .= "";
				...
				$Field                               = $this->_resource[$ProdID] ."_porcent";
				$CurrRow                             = array();
		...
		$parse['Production_of_resources_in_the_planet'] = str_replace('%s', $this->_current_planet['name'], $this->_lang['rs_production_on_planet']);

		$parse['production_level']		 = $this->prod_level ( $this->_current_planet['energy_used'] , $this->_current_planet['energy_max'] );
		...
		$parse['metal_max']              = $this->resource_color ( $this->_current_planet['metal'] , $this->_current_planet['metal_max'] );
		$parse['crystal_max']            = $this->resource_color ( $this->_current_planet['crystal'] , $this->_current_planet['crystal_max'] );
		$parse['deuterium_max']          = $this->resource_color ( $this->_current_planet['deuterium'] , $this->_current_planet['deuterium_max'] );

		$parse['metal_total']           = Format_Lib::color_number( Format_Lib::pretty_number( floor( ( ($this->_current_planet['metal_perhour']     * 0.01 * $parse['production_level'] ) + $parse['metal_basic_income']))));
		$parse['crystal_total']         = Format_Lib::color_number( Format_Lib::pretty_number( floor( ( ($this->_current_planet['crystal_perhour']   * 0.01 * $parse['production_level'] ) + $parse['crystal_basic_income']))));
		$parse['deuterium_total']       = Format_Lib::color_number( Format_Lib::pretty_number( floor( ( ($this->_current_planet['deuterium_perhour'] * 0.01 * $parse['production_level'] ) + $parse['deuterium_basic_income']))));
		$parse['energy_total']          = Format_Lib::color_number( Format_Lib::pretty_number( floor( ( $this->_current_planet['energy_max'] + $parse['energy_basic_income']    ) + $this->_current_planet['energy_used'] ) ) );


		$parse['daily_metal']			= $this->calculate_daily ( $this->_current_planet['metal_perhour'] , $parse['production_level'] , $parse['metal_basic_income'] );
		$parse['weekly_metal']			= $this->calculate_weekly ( $this->_current_planet['metal_perhour'] , $parse['production_level'] , $parse['metal_basic_income'] );


		$parse['daily_crystal']			= $this->calculate_daily ( $this->_current_planet['crystal_perhour'] , $parse['production_level'] , $parse['crystal_basic_income'] );
		$parse['weekly_crystal']		= $this->calculate_weekly ( $this->_current_planet['crystal_perhour'] , $parse['production_level'] , $parse['crystal_basic_income'] );


		$parse['daily_deuterium']		= $this->calculate_daily ( $this->_current_planet['deuterium_perhour'] , $parse['production_level'] , $parse['deuterium_basic_income'] );
		$parse['weekly_deuterium']		= $this->calculate_weekly ( $this->_current_planet['deuterium_perhour'] , $parse['production_level'] , $parse['deuterium_basic_income'] );
		...
		if ( $_POST && ! parent::$users->is_on_vacations ( $this->_current_user ) )
		{
			...
			parent::$db->query ( "UPDATE " . PLANETS . " SET
									`id` = '". $this->_current_planet['id'] ."'
									$SubQry
									WHERE `id` = '". $this->_current_planet['id'] ."';" );
