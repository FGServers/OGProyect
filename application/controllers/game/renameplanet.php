...
class RenamePlanet extends OGPCore
{
	const MODULE_ID	= 1;
	...
	{
		$parse 						= $this->_lang;
		$parse['planet_name'] 		= $this->_current_planet['name'];
		$parse['planet_id'] 		= $this->_current_planet['id'];
		$parse['galaxy_galaxy'] 	= $this->_current_planet['galaxy'];
	...
	private function rename_planet ( $new_name )
	{
		...
		if ( $new_name != "" )
		{
			parent::$db->query ( "UPDATE " . PLANETS . "
									SET `name` = '" . $new_name . "'
									WHERE `id` = '" . intval ( $this->_current_user['user_current_planet'] ) . "'
									LIMIT 1;");
		}
	}
	...
	private function delete_planet()
	{
		...
		$fleets_incoming	= parent::$db->query ( "SELECT *
														...
														WHERE ( fleet_owner = '" . intval ( $this->_current_user['user_id'] ) . "' AND
																fleet_start_galaxy='" . intval ( $this->_current_planet['galaxy'] ) . "' AND
																fleet_start_system='" . intval ( $this->_current_planet['system'] ) . "' AND
																fleet_start_planet='" . intval ( $this->_current_planet['planet'] ) . "') OR
															  ( fleet_target_owner = '" . intval ( $this->_current_user['user_id'] ) . "' AND
																fleet_end_galaxy='" . intval ( $this->_current_planet['galaxy'] ) . "' AND
																fleet_end_system='" . intval ( $this->_current_planet['system'] ) . "' AND
																fleet_end_planet='" . intval ( $this->_current_planet['planet'] ) . "')");
			...
			if ( sha1 ( $_POST['pw'] ) == $this->_current_user['password'] && $this->_current_user['user_home_planet_id'] != $this->_current_user['user_current_planet'])
			{
				if ( $this->_current_planet['moon_id'] != 0 )
				{
					parent::$db->query ( "UPDATE " . PLANETS . " AS p, " . PLANETS . " AS m, " . USERS . " AS u SET
													...
													WHERE p.`id` = '" . intval($this->_current_user['user_current_planet']) . "' AND
															m.`galaxy` = '" . intval($this->_current_planet['galaxy']) . "' AND
															m.`system` = '" . intval($this->_current_planet['system']) . "' AND
															m.`planet` = '" . intval($this->_current_planet['planet']) . "' AND
															m.`planet_type` = '3' AND
															u.`user_id` = '" . intval($this->_current_user['user_id']) . "';");
				}
				else
				{
