...
class Research extends OGPCore
{
	const MODULE_ID	= 6;
		...
		foreach ( $this->_lang['tech'] as $tech => $tech_name )
		{
			if ( $tech > 105 && $tech <= 199 )
			{
				if ( Developments_Lib::is_development_allowed ( $this->_current_user , $this->_current_planet , $tech ) )
				{
					...
					{
						if ( $this->_is_working['working_on']['b_tech_id'] == $tech )
						{
							$bloc	= $this->_lang;

							if ( $this->_is_working['working_on']['id'] != $this->_current_planet['id'] )
							{
								$bloc['tech_time']  = $this->_is_working['working_on']['b_tech'] - time();
								$bloc['tech_name']  = $this->_lang['bd_from'] . $this->_is_working['working_on']['name'] . '<br /> ' . Format_Lib::pretty_coords ( $this->_is_working['working_on']['galaxy'] , $this->_is_working['working_on']['system'] , $this->_is_working['working_on']['planet'] );
								$bloc['tech_home']  = $this->_is_working['working_on']['id'];
								$bloc['tech_id']    = $this->_is_working['working_on']['b_tech_id'];
							}
							else
							{
								$bloc['tech_time']  = $this->_current_planet['b_tech'] - time();
								$bloc['tech_name']  = '';
								$bloc['tech_home']  = $this->_current_planet['id'];
								$bloc['tech_id']    = $this->_current_planet['b_tech_id'];
							}
			...
			if ( in_array ( $technology , $this->_reslist['tech'] ) )
			{
				...
				switch ( $cmd )
				{
					...
					case 'cancel':
						...
						if ( $this->_is_working['working_on']['b_tech_id'] == $technology )
						{
							$costs                        						= Developments_Lib::development_price ( $this->_current_user , $working_planet , $technology );
							$working_planet['metal']      					   += $costs['metal'];
							$working_planet['crystal']    					   += $costs['crystal'];
							$working_planet['deuterium']  					   += $costs['deuterium'];
							$working_planet['b_tech_id']   						= 0;
							$working_planet['b_tech']      						= 0;
					...
					case 'search':
						...
						if ( Developments_Lib::is_development_allowed ( $this->_current_user , $working_planet , $technology ) && Developments_Lib::is_development_payable ( $this->_current_user , $working_planet , $technology ) && ! parent::$users->is_on_vacations ( $this->_current_user ) )
						{
							...
							$working_planet['crystal']    					   -= $costs['crystal'];
							$working_planet['deuterium']  					   -= $costs['deuterium'];
							$working_planet['b_tech_id']   						= $technology;
							$working_planet['b_tech']      						= time() + Developments_Lib::development_time ( $this->_current_user , $working_planet , $technology , FALSE , $this->_lab_level );
				...
				if ( $update_data == TRUE )
				{
					parent::$db->query ( "UPDATE " . PLANETS . " AS p, " . RESEARCH . " AS r SET
											p.`b_tech_id` = '" . $working_planet['b_tech_id'] . "',
											p.`b_tech` = '" . $working_planet['b_tech'] . "',
											p.`metal` = '" . $working_planet['metal'] . "',
											p.`crystal` = '" . $working_planet['crystal'] . "',
											p.`deuterium` = '" . $working_planet['deuterium'] . "',
											r.`research_current_research` = '" . $this->_current_user['research_current_research'] ."'
											WHERE p.`id` = '" . $working_planet['id'] . "'
		...
		if ( $this->_current_planet['planet_b_building_id'] != 0 )
		{
			...
			{	
				if(isset($arreglo[$i]))
				{
					...
					{
						$element_data	= explode ( "," , (isset($queue[$i] )));
	...
	private function handle_technologie_build()
	{
		...
		{
			if ( $this->_current_user['research_current_research'] != $this->_current_planet['id'] )
			{
				$working_planet = parent::$db->query_fetch ( "SELECT `id`, `name`, `b_tech`, `b_tech_id`, `galaxy`, `system`, `planet`
																FROM " . PLANETS . "
																WHERE `id` = '". intval ( $this->_current_user['research_current_research'] ) ."';");
			}
			...
			if ( $the_planet['b_tech'] <= time() && $the_planet['b_tech_id'] != 0 )
			{
				$the_planet['b_tech_id'] 	= 0;
	...
	private function set_labs_amount ()
	{
		...
		$labs_level = parent::$db->query_fetch ( "SELECT SUM(`building_laboratory`) AS `total_level`
														FROM " . BUILDINGS . " AS b
														INNER JOIN " . PLANETS . " AS p ON p.id = b.building_planet_id
														WHERE id_owner='" . (int)$this->_current_user['user_id'] . "'
