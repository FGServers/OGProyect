 ...
 * @version 1.0.1
 *****************************
 * @Archivo preferences.php.
 *****************************
 * @copyright Copyright (C) 2015.
 * @copyright EX XGProyect By Lucky (C) 2008 - 2014.
 */

class Options extends OGPCore
{
	const MODULE_ID	= 21;
		...
		if ($_POST && $mode == "exit")
		{
			if (isset($_POST["exit_modus"]) && $_POST["exit_modus"] == 'on' and $this->_current_user['setting_vacations_until'] <= time())
			{
				...
				parent::$db->query ( "UPDATE " . SETTINGS . ", " . PLANETS . " SET
										...
										building_metal_mine_porcent = '10',
										building_crystal_mine_porcent = '10',
										building_deuterium_sintetizer_porcent = '10',
										building_solar_plant_porcent = '10',
										building_fusion_reactor_porcent = '10',
										ship_solar_satellite_porcent = '10'
										WHERE `setting_user_id` = '".intval($this->_current_user['user_id'])."' AND id_owner = '".intval($this->_current_user['user_id'])."'" );
		...
		if ($_POST && $mode == "change")
		{
			...
			if (isset($_POST["urlaubs_modus"]) && $_POST["urlaubs_modus"] == 'on')
			{
				...
				parent::$db->query ( "UPDATE " . SETTINGS . ", " . PLANETS . " SET
										...
										metal_perhour = '".Functions_Lib::read_config ( 'metal_basic_income' )."',
										crystal_perhour = '".Functions_Lib::read_config ( 'crystal_basic_income' )."',
										deuterium_perhour = '".Functions_Lib::read_config ( 'deuterium_basic_income' )."',
										energy_used = '0',
										energy_max = '0',
										building_metal_mine_porcent = '0',
										building_crystal_mine_porcent = '0',
										building_deuterium_sintetizer_porcent = '0',
										building_solar_plant_porcent = '0',
										building_fusion_reactor_porcent = '0',
										ship_solar_satellite_porcent = '0'
										WHERE `setting_user_id` = '".intval($this->_current_user['user_id'])."' AND id_owner = '".intval($this->_current_user['user_id'])."'" );
			}
			else
			{
														...
														+
															(
																SELECT COUNT(id) AS quantity
																	FROM " . PLANETS . "
																		WHERE id_owner = '" . intval ( $this->_current_user['user_id'] ) . "' AND
																		(planet_b_building <> 0 OR b_tech <> 0 OR b_hangar <> 0)
															)
														) as total" );
