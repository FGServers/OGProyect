...
class Infos extends OGPCore
{
    ...
    private function DoFleetJump()
    {
        if ($_POST)
        {
            ...
            if ( $NextJumpTime == 0 )
            {
                ...
                $TargetGate   = parent::$db->query_fetch ( "SELECT `id`, `sprungtor`, `last_jump_time`
                											FROM " . PLANETS . "
                											WHERE `id` = '". $TargetPlanet ."';");
    ...
    private function BuildJumpableMoonCombo()
    {
        $MoonList        = parent::$db->query ( "SELECT *
        											...
        											WHERE `planet_type` = '3' AND
        													`id_owner` = '" . $this->_current_user['user_id'] . "';" );
        $Combo           = "";

        while ( $CurMoon = parent::$db->fetch_assoc ( $MoonList ) )
        {
            if ( $CurMoon['id'] != $this->_current_planet['id'] )
            {
    ...
    {
        $BuildLevelFactor 	= $this->_current_planet[ $this->_resource[$this->_element_id]."_porcent" ];
	...
	{
		return "<a href=\"game.php?page=galaxy&mode=3&galaxy=".$current_planet['galaxy']."&system=".$current_planet['system']."\">[".$current_planet['galaxy'].":".$current_planet['system'].":".$current_planet['planet']."]</a>";
	}
}
/* end of infos.php */