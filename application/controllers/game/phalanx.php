...
class Phalanx extends OGPCore
{
	...
	private function build_page()
    {
        $parse = $this->_lang;
        /* range */
        $radar_limit_inf = $this->_current_planet['system'] - $this->_formula->phalanx_range($this->_current_planet['building_phalanx']);
        $radar_limit_sup = $this->_current_planet['system'] + $this->_formula->phalanx_range($this->_current_planet['building_phalanx']);
        ...
        /* cheater detection */
        if ( $System < $radar_limit_inf or $System > $radar_limit_sup or $Galaxy != $this->_current_planet['galaxy'] or $PlType != 1 or $this->_current_planet['planet_type'] != 3 )
        {
        	Functions_Lib::redirect ( 'game.php?page=galaxy' );
        }

		/* main page */
        if ($this->_current_planet['deuterium'] > 10000)
        {
            parent::$db->query ( "UPDATE " . PLANETS . " SET
            						`deuterium` = `deuterium` - '10000'
            						WHERE `id` = '" . $this->_current_user['user_current_planet'] . "';");

            $TargetInfo 	= parent::$db->query_fetch ( "SELECT `name`, `id_owner`
            												FROM " . PLANETS . "
