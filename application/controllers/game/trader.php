			...
			switch ( $_POST['ress'] )
			{
				case 'metal':
				{
					...
					else
					{
						...
						if ( $this->_current_planet['metal'] > $necessaire )
						{
							parent::$db->query ( "UPDATE " . PLANETS . " SET
													`metal` = `metal` - " . round ( $necessaire ) . ",
													`crystal` = `crystal` + " . round ( $_POST['cristal'] ) .",
													`deuterium` = `deuterium` + " . round ( $_POST['deut'] ) ."
													WHERE `id` = '" . $this->_current_planet['id'] . "';" );

							$this->_current_planet['metal']     -= $necessaire;
				...
				case 'cristal':
				{
					...
					else
					{
						...
						if ( $this->_current_planet['crystal'] > $necessaire )
						{
							parent::$db->query ( "UPDATE " . PLANETS . " SET
													`metal` = `metal` + " . round ( $_POST['metal'] ) . ",
													`crystal` = `crystal` - " . round ( $necessaire ) . ",
													`deuterium` = `deuterium` + " . round ( $_POST['deut'] ) . "
													WHERE `id` = '".$this->_current_planet['id'] . "';" );

							$this->_current_planet['metal']     += isset ( $_POST['metal'] ) ? $_POST['metal'] : 0;
				...
				case 'deuterium':
				{
					...
					else
					{
						...
						if ( $this->_current_planet['deuterium'] > $necessaire )
						{
							parent::$db->query ( "UPDATE " . PLANETS . " SET
													`metal` = `metal` + " . round ( $_POST['metal'] ) . ",
													`crystal` = `crystal` + " . round ( $_POST['cristal'] ) . ",
													`deuterium` = `deuterium` - " . round ( $necessaire ) . "
													WHERE `id` = '" . $this->_current_planet['id'] . "';" );
