...
class Fleetshortcuts extends OGPCore
{
	const MODULE_ID	= 8;
	...
	private function build_page()
	{
		if(!empty($_GET['mode']))
		{
			...
			{
				$this->addFleetShortcuts(parent::$db->escape_value(strip_tags($_POST["name"])),intval($_POST["galaxy"]),intval($_POST["system"]),intval($_POST["planet"]),intval($_POST["moon"]));
			}
			elseif($mode=="edit" && isset($_GET['a']) && !empty($_POST['galaxy'])&&!empty($_POST['system'])&&!empty($_POST['planet']) )
			{
				$this->saveFleetShortcuts(intval($_GET['a']),parent::$db->escape_value(strip_tags($_POST["name"])),intval($_POST["galaxy"]),intval($_POST["system"]),intval($_POST["planet"]),intval($_POST["moon"]));
			}
