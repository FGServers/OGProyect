...
class Overview extends OGPCore
{
	const MODULE_ID	= 1;
	...
	private function build_page()
	{
		...
		$parse['planet_image'] 			= $this->_current_planet['image'];
		...
		$parse['planet_diameter'] 		= Format_Lib::pretty_number ( $this->_current_planet['diameter'] );
		$parse['planet_field_current'] 	= $this->_current_planet['field_current'];
		$parse['planet_field_max'] 		= Developments_Lib::max_fields ( $this->_current_planet );
		$parse['planet_temp_min'] 		= $this->_current_planet['temp_min'];
		$parse['planet_temp_max'] 		= $this->_current_planet['temp_max'];
	...
	private function get_fleet_movements()
	{
		...
		$own_fleets = parent::$db->query ( "SELECT *
											FROM " . FLEETS . "
											WHERE `fleet_owner` = '" . intval ( $this->_current_user['user_id'] ) . "' OR
													`fleet_target_owner` = '" . intval ( $this->_current_user['user_id'] ) . "';" );
	...
	private function get_planets()
	{
		...
		$planets_query 	= parent::$db->query ( "SELECT *
												...
												WHERE `id_owner` = '" . intval ( $this->_current_user['user_id'] ) . "'
		...
		while ( $user_planet = parent::$db->fetch_array ( $planets_query ) )
		{
			if ( $user_planet['id'] != $this->_current_user['user_current_planet'] && $user_planet['planet_type'] != 3)
			{
				...
				$planet_block .= Functions_Lib::set_url ( $url , $user_planet['name'] , Functions_Lib::set_image ( $image , $user_planet['name'] , $attributes ) );
				$planet_block .= '<center>';
