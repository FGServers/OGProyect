...
class Register extends OGPCore
{
	private $_lang;
	private $_creator;
		...
		if ( $_POST )
		{
			...
			else
			{
				...
				parent::$db->query ( "UPDATE " . USERS . " SET
										`user_home_planet_id` = (SELECT `id` FROM " . PLANETS . " WHERE `id_owner` = '" . $user_id . "' LIMIT 1),
										`user_current_planet` = (SELECT `id` FROM " . PLANETS . " WHERE `id_owner` = '" . $user_id . "' LIMIT 1),
										`user_galaxy` = '" . $galaxy . "',
										`user_system` = '" . $system . "',
										`user_planet` = '" . $planet . "'
