...
class Installation extends OGPCore
{
	private $_host;
	private $_db;
	private $_user;
	...
	{
		$config_file	= @fopen ( OGP_ROOT . 'application/config/config.php' , "w" );
		...
		$data 	 = "<?php\n";
		$data 	.= "if(!defined(\"INSIDE\")){ header(\"location:".OGP_ROOT."\"); }\n";
		...
		$data 	.= "defined('SECRETWORD') ? NULL : define('SECRETWORD', 'ogp-".$this->generate_token()."');\n";
		$data	.= "?>";
	...
	{
		$needed_config_file		= @fopen ( OGP_ROOT . 'application/config/config.xml' , "r" );
		$default_config_file	= @fopen ( OGP_ROOT . 'application/config/config.xml.cfg' , "r" );
	...
	{
		$location					= OGP_ROOT . 'application/config/';
