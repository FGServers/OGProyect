...
class Maker extends OGPCore
{
		...
		if ( isset ( $_POST['add_alliance'] ) && $_POST['add_alliance'] )
		{
			...
			if ( ! $check_alliance &&  ! empty ( $alliance_founder ) && $alliance_founder > 0 )
			{
				parent::$db->query ( "INSERT INTO `" . ALLIANCE . "` SET
										...
										`ally_owner`='" . $alliance_founder . "',
										`ally_owner_range` = '" . $this->_lang['mk_alliance_founder_rank'] . "',
	...
	private function make_moon()
	{
		...
		if ( isset ( $_POST['add_moon'] ) && $_POST['add_moon'] )
		{
			...
			$max_fields		= (int)$_POST['field_max'];

			$moon_planet	= 	parent::$db->query_fetch ( "SELECT p.*, (SELECT `id`
																			FROM " . PLANETS . "
																			WHERE `galaxy` = (SELECT `galaxy`
																								FROM " . PLANETS . "
																								WHERE `id` = '" . $planet_id . "'
																										AND `planet_type` = 1)
																					AND `system` = (SELECT `system`
																										FROM " . PLANETS . "
																										WHERE `id` = '" . $planet_id . "'
																											AND `planet_type` = 1)
																					AND `planet` = (SELECT `planet`
																										FROM " . PLANETS . "
																										WHERE `id` = '" . $planet_id . "'
																											AND `planet_type` = 1)
																					AND `planet_type` = 3) AS id_moon
																FROM " . PLANETS . " AS p
																WHERE p.`id` = '" . $planet_id . "' AND
																		p.`planet_type` = '1'" );
			...
			if ( $moon_planet && is_numeric ( $planet_id ) )
			{
				...
				{
					$galaxy    = $moon_planet['galaxy'];
					$system    = $moon_planet['system'];
					$planet    = $moon_planet['planet'];
					$owner     = $moon_planet['id_owner'];
					...
					{
						$maxtemp		= $moon_planet['temp_max'] - mt_rand(10, 45);
						$mintemp		= $moon_planet['temp_min'] - mt_rand(10, 45);
					}
	...
	private function build_planet_combo()
	{
		...
		while ( $planets_row = parent::$db->fetch_array ( $planets ) )
		{
			...
			{
				$combo_rows	.= '<option value="' . $planets_row['id'] . '" ' . ( $_GET['planet'] == $planets_row['id'] ? 'selected' : '' ) . ' >' .  $planets_row['name'] . ' [' . $planets_row['galaxy'] . ':' . $planets_row['system'] . ':' . $planets_row['planet'] . ']' . '</option>';
			}
			else
			{
