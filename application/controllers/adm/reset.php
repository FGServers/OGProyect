...
class Reset extends OGPCore
{
	...
	private function reset_universe ()
	{
		...
		while ( $TheUser = parent::$db->fetch_assoc ( $AllUsers ) )
		{
			...
			{
				$UserPlanet     = parent::$db->query_fetch ( "SELECT `name`
																FROM " . PLANETS . "_s
																WHERE `id` = '". $TheUser['user_home_planet_id']."';" );
				if ($UserPlanet['name'] != "")
				{
					$Time	=	time();
					...
					$this->_creator->create_planet ( $TheUser['user_galaxy'] , $TheUser['user_system'] , $TheUser['user_planet'] , $NewUser , $UserPlanet['name'] , TRUE );

					$PlanetID       = parent::$db->query_fetch ( "SELECT `id`
																	FROM " . PLANETS . "
																	WHERE `id_owner` = '". $NewUser ."'
																	LIMIT 1;" );
			...
			if ($_POST['resetall']	!=	'on')
			{
				...
				if ($_POST['edif_p']	==	'on')
				{
					parent::$db->query ( "UPDATE " . BUILDINGS . " AS b
											INNER JOIN " . PLANETS . " AS p ON b.building_planet_id = p.id SET
