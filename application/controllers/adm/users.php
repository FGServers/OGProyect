...
class Users extends OGPCore
{
	...
	private function get_data_info()
	{
		...
		$parse['onlinetime']								= $this->last_activity ( $this->_user_query['user_onlinetime'] );
	...
	private function get_data_planets()
	{
		...
		if (  $this->_planet > 0 )
		{
			$sub_query 	= ' AND p.`id` = ' . $this->_planet;
		}
		...
		switch ( $this->_edit )
		{
			...
			case 'delete':
				...
				parent::$db->query ( "UPDATE " . PLANETS . " AS p, " . PLANETS . " AS m, " . USERS . " AS u SET
													...
													WHERE p.`id` = '" . intval($this->_planet) . "' AND
															m.`galaxy` = p.`galaxy` AND
															m.`system` = p.`system` AND
															m.`planet` = p.`planet` AND
															m.`planet_type` = '3';");
	...
	private function get_data_moons()
	{
		...
		if (  $this->_moon > 0 )
		{
			$sub_query 	= ' AND m.`id` = ' . $this->_moon;
		}

		$moons_query 	= parent::$db->query ( "SELECT {$get_query}
													FROM " . PLANETS . " AS m
													INNER JOIN " . BUILDINGS . " AS b ON b.building_planet_id = m.id
													INNER JOIN " . DEFENSES . " AS d ON d.defense_planet_id = m.id
													INNER JOIN " . SHIPS . " AS s ON s.ship_planet_id = m.id
													WHERE m.`id_owner` = '" . $this->_id . "'
		...
		switch ( $this->_edit )
		{
			...
			case 'delete':
				...
				parent::$db->query ( "UPDATE " . PLANETS . " AS m, " . USERS . " AS u SET
													m.`planet_destroyed` = '" . (time() + (PLANETS_LIFE_TIME * 3600)) . "',
													u.`user_current_planet` = u.`user_home_planet_id`
													WHERE m.`id` = '" . intval ( $this->_moon ) . "' AND
															m.`planet_type` = '3';");
				$this->refresh_page();
	...
	private function save_settings()
	{
		...
		$setting_planet_sort		= ( ( isset ( $_POST['setting_planet_sort'] ) ) ? intval ( $_POST['setting_planet_sort'] ) : 0 );
		$setting_planet_order		= ( ( isset ( $_POST['setting_planet_order'] ) ) ? intval ( $_POST['setting_planet_order'] ) : 0 );
		$setting_no_ip_check		= ( ( isset ( $_POST['setting_no_ip_check'] ) && $_POST['setting_no_ip_check'] == 'on' ) ? 1 : 0 );
		$setting_probes_amount		= ( ( isset ( $_POST['setting_probes_amount'] ) ) ? intval ( $_POST['setting_probes_amount'] ) : 0 );
		$setting_fleet_actions		= ( ( isset ( $_POST['setting_fleet_actions'] ) ) ? intval ( $_POST['setting_fleet_actions'] ) : 0 );
		...
		if ( $this->_user_query['setting_vacations_status'] == 1 && $setting_vacations_status == 0 )
		{
			...
			$vacation_condition = " AND p.`id_owner` = '".intval($this->_id)."'";
			$vacation_query 	= "
			s.`setting_vacations_status` = '{$setting_vacations_status}',
			s.`setting_vacations_until` = '{$setting_vacations_until}',
			p.`building_metal_mine_porcent` = '10',
			p.`building_crystal_mine_porcent` = '10',
			p.`building_deuterium_sintetizer_porcent` = '10',
			p.`building_solar_plant_porcent` = '10',
			p.`building_fusion_reactor_porcent` = '10',
			p.`ship_solar_satellite_porcent` = '10',";
		}
		elseif ( $this->_user_query['setting_vacations_status'] == 0 && $setting_vacations_status == 1 )
		{
			...
			s.`setting_vacations_status` = '{$setting_vacations_status}',
			s.`setting_vacations_until` = '{$setting_vacations_until}',
			p.`metal_perhour` = '".Functions_Lib::read_config ( 'metal_basic_income' )."',
			p.`crystal_perhour` = '".Functions_Lib::read_config ( 'crystal_basic_income' )."',
			p.`deuterium_perhour` = '".Functions_Lib::read_config ( 'deuterium_basic_income' )."',
			p.`energy_used` = '0',
			p.`energy_max` = '0',
			p.`building_metal_mine_porcent` = '0',
			p.`building_crystal_mine_porcent` = '0',
			p.`building_deuterium_sintetizer_porcent` = '0',
			p.`building_solar_plant_porcent` = '0',
			p.`building_fusion_reactor_porcent` = '0',
			p.`ship_solar_satellite_porcent` = '0',";
		}
		else
		{
			$vacation_head		= '';
	...
	private function save_planet ( $type = 1 )
	{
		...
		// QUERY END
		$query_string  .= " WHERE `id` = '" . parent::$db->escape_value ( $id_get ) . "';";
	...
	{
		$combo_rows	= '';
		$planets	= parent::$db->query ( "SELECT `id`, `name`, `galaxy`, `system`, `planet`
												FROM " . PLANETS . "
												WHERE id_owner = '" . $this->_id . "';" );

		while ( $planets_row = parent::$db->fetch_array ( $planets ) )
		{
			if ( $user_data[$id_field] == $planets_row['id'] )
			{
				$combo_rows	.= '<option value="' . $planets_row['id'] . '" selected>' .  $planets_row['name'] . ' [' . $planets_row['galaxy'] . ':' . $planets_row['system'] . ':' . $planets_row['planet'] . ']' . '</option>';
			}
			else
			{
				$combo_rows	.= '<option value="' . $planets_row['id'] . '">' .  $planets_row['name'] . ' [' . $planets_row['galaxy'] . ':' . $planets_row['system'] . ':' . $planets_row['planet'] . ']' . '</option>';
			}
		}
	...
	{
		$combo_rows	= '';
		$alliances	= parent::$db->query ( "SELECT `alliance_id`, `alliance_name`, `ally_tag`
												FROM " . ALLIANCE . ";" );
		...
		while ( $alliance_row = parent::$db->fetch_array ( $alliances ) )
		{
			...
			{
				$combo_rows	.= '<option value="' . $alliance_row['alliance_id'] . '" selected>' .  $alliance_row['alliance_name'] . ' [' . $alliance_row['ally_tag'] . ']' . '</option>';
			}
			else
			{
				$combo_rows	.= '<option value="' . $alliance_row['alliance_id'] . '">' .  $alliance_row['alliance_name'] . ' [' . $alliance_row['ally_tag'] . ']' . '</option>';
			}
		}
	...
	private function planets_table ( $planets_data )
	{
		...
		{
			$parse['planet_id']			= $planets['id'];
			$parse['planet_name']		= $planets['name'];
			$parse['planet_image']		= $planets['image'];
			$parse['planet_image']		= $planets['image'];
	...
	private function moons_table ( $moons_data )
	{
		...
		{
			$parse['moon_id']		= $moons['id'];
			$parse['moon_name']		= str_replace ( '%s' , $moons['name'] , $this->_lang['us_user_moon_title'] );
			$parse['moon_image']	= $moons['image'];
	...
	*/
	private function edit_main ( $planets_data )
	{
		...
		$parse['id_owner']								= $this->build_users_combo ( $parse['id_owner'] );
		$parse['last_update']							= date ( Functions_Lib::read_config ( 'date_format_extended' ) , $parse['last_update'] );
		...
		$parse['b_tech']								= $parse['b_tech'] > 0 ? date ( Functions_Lib::read_config ( 'date_format_extended' ) , $parse['b_tech'] ) : '-';
		$parse['b_hangar']								= $parse['b_hangar'] > 0 ? date ( Functions_Lib::read_config ( 'date_format_extended' ) , $parse['b_hangar'] ) : '-';
		$parse['image']									= $this->build_image_combo ( $parse['image'] );
		$parse['building_metal_mine_porcent']			= $this->build_percent_combo ( $parse['building_metal_mine_porcent'] );
		$parse['building_crystal_mine_porcent']			= $this->build_percent_combo ( $parse['building_crystal_mine_porcent'] );
		$parse['building_deuterium_sintetizer_porcent']	= $this->build_percent_combo ( $parse['building_deuterium_sintetizer_porcent'] );
		$parse['building_solar_plant_porcent']			= $this->build_percent_combo ( $parse['building_solar_plant_porcent'] );
		$parse['building_fusion_reactor_porcent']		= $this->build_percent_combo ( $parse['building_fusion_reactor_porcent'] );
		$parse['ship_solar_satellite_porcent']			= $this->build_percent_combo ( $parse['ship_solar_satellite_porcent'] );
		$parse['last_jump_time']						= $parse['last_jump_time'] > 0 ? date ( Functions_Lib::read_config ( 'date_format_extended' ) , $parse['last_jump_time'] ) : '-';
		$parse['invisible_start_time']					= $parse['invisible_start_time'] > 0 ? date ( Functions_Lib::read_config ( 'date_format_extended' ) , $parse['invisible_start_time'] ) : '-';
