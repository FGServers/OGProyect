...
class Home extends OGPCore
{
	...
	private function build_page()
	{
		...
		{
			if ( is_writable ( OGP_ROOT . 'application/config/config.php' ) )
			{
				$message[1]		= $this->_lang['hm_config_file_writable'] . '<br />';
				$error++;
			}

			if ( ( filesize ( OGP_ROOT . LOGS_PATH . 'ErrorLog.php' ) ) != 0 )
			{
		...
		$parse['old_version_alert']	= ( $old_version ) ? '<a href="http://www.ogproyect.com/downloads/">' . $this->_lang['hm_update'] . '</a> <i class="icon-download"></i>' : '';
	...
	private function check_updates()
	{
		...
		{
			//$last_v 	= @file_get_contents ( 'http://ogproyect.com/current.php' );
			$last_v 	= '1.0.2';
