	...
	public function attack_mission ( $fleet_row )
	{
		if ($fleet_row['fleet_mess'] == 0 && $fleet_row['fleet_start_time'] <= time())
		{
			$targetPlanet = parent::$db->query_fetch ( "SELECT *
														...
														WHERE `galaxy` = ". intval($fleet_row['fleet_end_galaxy']) ." AND
																`system` = ". intval($fleet_row['fleet_end_system']) ." AND
																`planet_type` = ". intval($fleet_row['fleet_end_type']) ." AND
																`planet` = ". intval($fleet_row['fleet_end_planet']) .";" );
			...
			$targetUser   = parent::$db->query_fetch ( 'SELECT *
															FROM ' . USERS . '
															WHERE user_id = '.intval($targetPlanet['id_owner']) );

			UpdateResources_Lib::update_resource ( $targetUser, $targetPlanet, time() );

			$targetGalaxy = parent::$db->query_fetch ( 'SELECT `id`
														FROM ' . PLANETS . '
														WHERE `galaxy` = '. intval($fleet_row['fleet_end_galaxy']) .' AND
																`system` = '. intval($fleet_row['fleet_end_system']) .' AND
																`planet` = '. intval($fleet_row['fleet_end_planet']) .' AND
																`planet_type` = 3;' );
			...
			$targetUser   = parent::$db->query_fetch ( 'SELECT 	u.*,
														...
														WHERE id='.intval($targetPlanet['id_owner']) );
			...
			for ($i = 200; $i < 500; $i++)
			{
				if (isset(parent::$_resource[$i]) && isset($targetPlanet[parent::$_resource[$i]]))
				{
					$defense[0]['def'][$i] = $targetPlanet[parent::$_resource[$i]];
				}
			}
			...
			foreach ($defense as $fleetID => $defender)
			{
				...
				else
				{
					...
					foreach ($defender['def'] as $element => $amount)
					{
						$fleetArray .= '`'.parent::$_resource[$element].'`='.$amount.', ';
					}
			...
			{
				include_once ( OGP_ROOT . 'application/libraries/Creator_Lib.php' );
		...
		elseif ($fleet_row['fleet_end_time'] <= time())
		{
			$Message         = sprintf( $this->_lang['sys_fleet_won'],
						$TargetName, Fleets_Lib::target_link($fleet_row, ''),
	...
	*/
	private function build_report ( &$result_array , &$steal_array , &$moon_int , &$moon_string , &$time_float )
	{
		...
		elseif ($result_array['won'] == "a")
		{
			...
			$result1 .= $this->_lang['sys_stealed_ressources']." ".$steal_array['metal']." ".$this->_lang['Metal'].", ".$steal_array['crystal']." ".$this->_lang['Crystal']." ".$this->_lang['and']." ".$steal_array['deuterium']." ".$this->_lang['Deuterium']."<br />";
		}
		else
		{
	...
	*/
	private function acs_attack ( &$attackers , &$defenders )
    {
        ...
        foreach ($attackers as $fleetID => $attacker) {
            foreach ($attacker['detail'] as $element => $amount) {
                $resourcePointsAttacker['metal'] += parent::$_pricelist[$element]['metal'] * $amount;
                $resourcePointsAttacker['crystal'] += parent::$_pricelist[$element]['crystal'] * $amount ;

                $totalResourcePoints['attacker'] += parent::$_pricelist[$element]['metal'] * $amount ;
                $totalResourcePoints['attacker'] += parent::$_pricelist[$element]['crystal'] * $amount ;
            }
        }
        ...
        foreach ($defenders as $fleetID => $defender) {
            foreach ($defender['def'] as $element => $amount) {                                //Line20
                if ($element < 300) {
                    $resourcePointsDefender['metal'] += parent::$_pricelist[$element]['metal'] * $amount ;
                    $resourcePointsDefender['crystal'] += parent::$_pricelist[$element]['crystal'] * $amount ;

                    $totalResourcePoints['defender'] += parent::$_pricelist[$element]['metal'] * $amount ;
                    $totalResourcePoints['defender'] += parent::$_pricelist[$element]['crystal'] * $amount ;
                } else {
        ...
        for ($round = 0, $rounds = array(); $round < $max_rounds; $round++) {
            ...
            foreach ($attackers as $fleetID => $attacker) {
                ...
                foreach ($attacker['detail'] as $element => $amount) {
                    ...
                    $thisAtt    = $amount * (parent::$_combat_caps[$element]['attack']) * $attTech * (mt_rand(80, 120) / 100); //attaque
                    $thisDef    = $amount * (parent::$_combat_caps[$element]['shield']) * $defTech ; //bouclier
                    $thisShield    = $amount * (parent::$_pricelist[$element]['metal'] + parent::$_pricelist[$element]['crystal']) / 10 * $shieldTech; //coque
                ...
                foreach($attacker['detail'] as $element => $amount) {
                    ...
                    if ($amount > 0) {
                        if ($attArray[$fleetID][$element]['def']/$amount < $defender_moc) {
                            ...
                            $ile_removePoints = floor($defender_moc / ((parent::$_pricelist[$element]['metal'] + parent::$_pricelist[$element]['crystal'])  / 10));
                ...
                foreach($defender['def'] as $element => $amount) {
                    ...
                    if ($amount > 0) {
                        if ($defArray[$fleetID][$element]['def']/$amount < $attacker_moc) {
                            ...
                            $ile_removePoints = floor($attacker_moc / ((parent::$_pricelist[$element]['metal'] + parent::$_pricelist[$element]['crystal']) / 10));
            ...
            foreach ($attackers as $fleetID => $attacker) {
                foreach ($defenders as $fleetID2 => $defender) {
                    foreach($attacker['detail'] as $element => $amount) {
                        if ($amount > 0) {
                            foreach (parent::$_combat_caps[$element]['sd'] as $c => $d) {
        ...
        foreach ($defenders as $fleetID => $defender) {
            foreach ($defender['def'] as $element => $amount) {
                if ($element < 300) {                                                        // flotte defenseur en CDR
                    ...
                    $resourcePointsDefenderDefs['metal'] += parent::$_pricelist[$element]['metal'] * ($lost - $giveback) ;
                    $resourcePointsDefenderDefs['crystal'] += parent::$_pricelist[$element]['crystal'] * ($lost - $giveback) ;
		...
		foreach ($attackFleets as $FleetID => $Attacker)
		{
			foreach($Attacker['detail'] as $Element => $amount)
			{
				if ($Element != 210) //fix probos capacity in attack by jstar
					$SortFleets[$FleetID]        += parent::$_pricelist[$Element]['capacity'] * $amount;
			}
