...
class Attack extends Missions
{
	...
	*/
	public function attack_mission ( $fleet_row )
	{
		...
		{
			require ( OGP_ROOT . BATTLE_ENGINE_CORE . 'utils/includer.php' );
			require ( OGP_ROOT . BATTLE_ENGINE_CORE . 'utils/LangImplementation.php' );
			...
			$target_planet = parent::$db->query_fetch ( "SELECT *
														...
														WHERE `galaxy` = ". (int)$fleet_row['fleet_end_galaxy'] ." AND
																`system` = ". (int)$fleet_row['fleet_end_system'] ." AND
																`planet_type` = ". (int)$fleet_row['fleet_end_type'] ." AND
																`planet` = ". (int)$fleet_row['fleet_end_planet'] .";" );
			...
			if ( $fleet_row['fleet_group'] != 0 )
			{
				...
				$attackers 	= $this->get_player_group_from_query($fleets);
			}
			else
			{
				$attackers 	= $this->get_player_group ( $fleet_row );
			}

			//defenders fleet sum
			$def = parent::$db->query_fetch ( 'SELECT *
													FROM ' . FLEETS . '
														WHERE `fleet_end_galaxy` = '. (int)$fleet_row['fleet_end_galaxy'] .' AND
															`fleet_end_system` = '. (int)$fleet_row['fleet_end_system'] .' AND
															`fleet_end_type` = '. (int)$fleet_row['fleet_end_type'] .' AND
															`fleet_end_planet` = '. (int)$fleet_row['fleet_end_planet'] .' AND
															fleet_start_time<'.time().' AND
															fleet_end_stay>='.time() );

			$defenders 		= $this->get_player_group_from_query ( $def , TRUE , $target_user );
			...
			for ( $i = self::DEFENSE_MIN_ID ; $i < self::DEFENSE_MAX_ID ; $i++ )
			{
				if ( isset ( $this->_resource[$i] ) && isset ( $target_planet[$this->_resource[$i]] ) )
				{
					...
					{
						$homeFleet->add ( $this->get_ship_type ( $i , $target_planet[$this->_resource[$i]] ) );
					}
				}
			}
			...
			if ( !$defenders->existPlayer( $target_userID ) )
			{
				...
				$player->setName ( $target_user['name'] );
	...
	private function update_moon ( $fleet_row , $report , $moonName , $target_userId , $target_planet )
	{
		...
		$moon_exists	= parent::$db->query_fetch ( "SELECT `id`
														FROM `" . PLANETS . "`
														WHERE `galaxy` = '" . $galaxy . "'
															AND `system` = '" . $system . "'
															AND `planet` = '" . $planet . "'
