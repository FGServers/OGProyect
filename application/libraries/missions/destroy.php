...
class Destroy extends Missions
{
	...
	*/
	public function destroy_mission ( $fleet_row )
	{
		...
		{
			$current_data	= parent::$db->query_fetch ( "SELECT p.name, r.research_weapons_technology, r.research_shielding_technology, r.research_armour_technology, u.user_name, u.user_id
															FROM " . PLANETS . " AS p
															INNER JOIN " . USERS . " AS u ON u.user_id = p.id_owner
															INNER JOIN " . PREMIUM . " AS pr ON pr.premium_user_id = p.id_owner
															INNER JOIN " . RESEARCH . " AS r ON r.research_user_id = p.id_owner
															WHERE p.`galaxy` = " . $fleet_row['fleet_start_galaxy'] . " AND
																	p.`system` = " . $fleet_row['fleet_start_system'] . " AND
																	p.`planet` = " . $fleet_row['fleet_start_planet'] . " AND
																	p.`planet_type` = " . $fleet_row['fleet_start_type'] . ";" );

			$target_data	= parent::$db->query_fetch ( "SELECT s.*, d.*, p.id, p.diameter, p.id_owner, u.user_name, u.user_current_planet, r.research_weapons_technology, r.research_shielding_technology, r.research_armour_technology
															FROM " . PLANETS . " AS p
															...
															INNER JOIN " . PREMIUM . " AS pr ON pr.premium_user_id = p.id_owner
															INNER JOIN " . RESEARCH . " AS r ON r.research_user_id = p.id_owner
															WHERE p.`galaxy` = '" . $fleet_row['fleet_end_galaxy'] . "' AND
			...
			{
				if ( $target_data[parent::$_resource[$SetItem]] > 0 )
				{
					$target_ships[$SetItem]['count'] = $target_data[parent::$_resource[$SetItem]];
				}
			}
			...
			{
				$FleetStorage += parent::$_pricelist[$Ship]["capacity"] * $Count['count'];
			...
			if ( ! is_null ( $target_ships ) )
			{
				...
				{
					$TargetPlanetUpd	.= "`" . parent::$_resource[$Ship] . "` = '" . $Count['count'] . "', ";
				}
			}

			if ( $FleetResult == "a" )
			{
				$destructionl1	= 100 - sqrt ( $target_data['diameter'] );
				...
				if ( $tirage <= $chance )
				{
					...
					parent::$db->query ( "UPDATE " . PLANETS . " AS p SET
											...
											WHERE p.`id` = '" . $target_data['id'] . "';" );

					if ( $target_data['user_current_planet'] == $target_data['id'] )
					{
						parent::$db->query ( "UPDATE " . USERS . " SET
												`user_current_planet` = (SELECT `id`
																	FROM " . PLANETS . "
																	WHERE `galaxy` = '" . $fleet_row['fleet_end_galaxy'] . "' AND
																			`system` = '" . $fleet_row['fleet_end_system'] . "' AND
																			`planet` = '" . $fleet_row['fleet_end_planet'] . "' AND
																			`planet_type` = '1')
												WHERE `user_id` = '" . $target_data['id_owner'] . "';" );
					}
				}
				else
				{
				...
				$destructionrip	= sqrt ( $target_data['diameter'] ) / 2;
			...
			$introdestruc			= sprintf ( $this->_lang['sys_destruc_mess'] , $current_data['name'] , $fleet_row['fleet_start_galaxy'] , $fleet_row['fleet_start_system'] , $fleet_row['fleet_start_planet'] , $fleet_row['fleet_end_galaxy'] , $fleet_row['fleet_end_system'] , $fleet_row['fleet_end_planet'] );

			parent::$db->query ( "UPDATE " . PLANETS . " AS p
									INNER JOIN " . SHIPS . " AS s ON s.ship_planet_id = p.id
									INNER JOIN " . DEFENSES . " AS d ON d.defense_planet_id = p.id SET
									$TargetPlanetUpd
									`metal` = `metal` - '". $Mining['metal'] ."',
									`crystal` = `crystal` - '". $Mining['crystal'] ."',
									`deuterium` = `deuterium` - '". $Mining['deuter'] ."',
			...
			$owners				= $fleet_row['fleet_owner'].",".$target_data['id_owner'];
			...
			parent::$db->query ( "UPDATE " . FLEETS . " SET
									...
									`fleet_mess` = '1'
									WHERE fleet_id = '" . intval($fleet_row['fleet_id']) . "';" );
			...
			$this->destroy_message ( $target_data['id_owner'] , $raport2 , $fleet_row['fleet_start_time'] );
/*
		$fquery = "";
		...
		if ( $fleet_row['fleet_end_time'] <= time() )
		{
			if ( !is_null ( $current_ships ) )
			{
				foreach ( $current_ships as $Ship => $Count )
				{
					$fquery .= "`". parent::$_resource[$Ship] ."` = `". parent::$_resource[$Ship] ."` + '". $Count['count'] ."', ";
				}
			}
			else
			{
				...
				foreach ( $fleet as $a => $b )
				{
					...
					{
						$a 			= explode ( "," , $b );
						$fquery    .= "{parent::$_resource[$a[0]]}={parent::$_resource[$a[0]]} + {$a[1]}, \n";
					}
				}
			}
	...
	*/
	private function attack ( $current_ships , $target_ships , $current_tech , $target_tech )
	{
		...
		if ( ! is_null ( $current_ships ) )
		{
			...
			{
				$current_debris_start['metal']   = $current_debris_start['metal']   + $current_ships[$a]['count'] * parent::$_pricelist[$a]['metal'];
				$current_debris_start['crystal'] = $current_debris_start['crystal'] + $current_ships[$a]['count'] * parent::$_pricelist[$a]['crystal'];
			}
		}
		...
		if ( !is_null ( $target_ships ) )
		{
			foreach ( $target_ships as $a => $b )
			{
				if ( $a < 300 )
				{
					$target_debris_start['metal']   		= $target_debris_start['metal']   + $target_ships[$a]['count'] * parent::$_pricelist[$a]['metal'];
					$target_debris_start['crystal'] 		= $target_debris_start['crystal'] + $target_ships[$a]['count'] * parent::$_pricelist[$a]['crystal'];
				}
				else
				{
					$target_debris_start_defense['metal']   = $target_debris_start_defense['metal']   + $target_ships[$a]['count'] * parent::$_pricelist[$a]['metal'];
					$target_debris_start_defense['crystal'] = $target_debris_start_defense['crystal'] + $target_ships[$a]['count'] * parent::$_pricelist[$a]['crystal'];
				}
			}
		}
		...
		for ( $i = 1 ; $i <= 7 ; $i++ )
		{
			...
			if ( ! is_null ( $current_ships ) )
			{
				foreach ( $current_ships as $a => $b )
				{
					$current_ships[$a]["defense"]	= $current_ships[$a]['count'] * ( parent::$_pricelist[$a]['metal'] + parent::$_pricelist[$a]['crystal'] ) / 10 * ( 1 + ( 0.1 * ( $current_tech["research_shielding_technology"] ) ) );
					$rand 							= mt_rand ( 80 , 120 ) / 100;
					$current_ships[$a]["shield"] 	= $current_ships[$a]['count'] * parent::$_combat_caps[$a]['shield'] * ( 1 + ( 0.1 * $current_tech["research_armour_technology"] ) ) * $rand;
					$atak_statku 					= parent::$_combat_caps[$a]['attack'];
			...
			if ( ! is_null ( $target_ships ) )
			{
				foreach ( $target_ships as $a => $b )
				{
					$target_ships[$a]["defense"]	= $target_ships[$a]['count'] * ( parent::$_pricelist[$a]['metal'] + parent::$_pricelist[$a]['crystal'] ) / 10 * ( 1 + ( 0.1 * ( $target_tech["research_shielding_technology"] ) ) );
					$rand 							= mt_rand ( 80 , 120 ) / 100;
					$target_ships[$a]["shield"] 	= $target_ships[$a]['count'] * parent::$_combat_caps[$a]['shield'] * ( 1 + ( 0.1 * $target_tech["research_armour_technology"] ) ) * $rand;
					$atak_statku 					= parent::$_combat_caps[$a]['attack'];
			...
			foreach ( $current_ships as $a => $b )
			{
				if ( $attacker_amount > 0 )
				{
					...
					if ( $current_ships[$a]["shield"] < $wrog_moc )
					{
						...
						$ile_zdjac 			= floor ( ( $wrog_moc / ( ( parent::$_pricelist[$a]['metal'] + parent::$_pricelist[$a]['crystal'] ) / 10 ) ) );
			...
			foreach ( $target_ships as $a => $b )
			{
				if ( $enemy_amount > 0 )
				{
					...
					if ( $target_ships[$a]["shield"] < $atakujacy_moc )
					{
						...
						$ile_zdjac 		= floor ( ( $atakujacy_moc / ( ( parent::$_pricelist[$a]['metal'] + parent::$_pricelist[$a]['crystal'] ) / 10 ) ) );
			...
			foreach ( $current_ships as $a => $b )
			{
				foreach ( parent::$_combat_caps[$a]['sd'] as $c => $d )
				{
		...
		if ( ! is_null ( $current_ships ) )
		{
			foreach ( $current_ships as $a => $b )
			{
				$current_debris_end['metal']	= $current_debris_end['metal'] + $current_ships[$a]['count'] * parent::$_pricelist[$a]['metal'];
				$current_debris_end['crystal'] 	= $current_debris_end['crystal'] + $current_ships[$a]['count'] * parent::$_pricelist[$a]['crystal'];
			}
		}
		...
		if ( ! is_null ( $target_ships ) )
		{
			foreach ( $target_ships as $a => $b )
			{
				if ( $a < 300 )
				{
					$target_debris_end['metal'] 			= $target_debris_end['metal'] + $target_ships[$a]['count'] * parent::$_pricelist[$a]['metal'];
					$target_debris_end['crystal'] 			= $target_debris_end['crystal'] + $target_ships[$a]['count'] * parent::$_pricelist[$a]['crystal'];
				}
				else
				{
					$target_debris_end_obrona['metal'] 		= $target_debris_end_obrona['metal'] + $target_ships[$a]['count'] * parent::$_pricelist[$a]['metal'];
					$target_debris_end_obrona['crystal'] 	= $target_debris_end_obrona['crystal'] + $target_ships[$a]['count'] * parent::$_pricelist[$a]['crystal'];
				}
			}
		}
		...
		if ( ! is_null ( $target_ships ) )
		{
			foreach ( $target_ships as $a => $b )
			{
				if ( $a > 300 )
				{
					$straty_obrona_wrog			= $straty_obrona_wrog + ( ( $target_start[$a]['count'] - $target_ships[$a]['count'] ) * ( parent::$_pricelist[$a]['metal'] + parent::$_pricelist[$a]['crystal'] ) );
