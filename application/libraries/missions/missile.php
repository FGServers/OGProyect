	...
	*/
	public function missile_mission ( $fleet_row )
	{
		if ( $fleet_row['fleet_start_time'] <= time() )
		{
			...
			{
				$attacker_data	= parent::$db->query_fetch ( "SELECT p.`name`, r.`research_weapons_technology`
																FROM " . PLANETS . " AS p
																INNER JOIN " . RESEARCH . " AS r ON r.research_user_id = p.id_owner
																WHERE `galaxy` = " . $fleet_row['fleet_start_galaxy'] . " AND
																		`system` = " . $fleet_row['fleet_start_system'] . " AND
																		`planet` = " . $fleet_row['fleet_start_planet'] . " AND
																		`planet_type` = " . $fleet_row['fleet_start_type'] . ";" );

				$target_data	= parent::$db->query_fetch ( "SELECT p.`id`, p.`name`, p.`id_owner`, d.*, r.`research_shielding_technology`
																FROM " . PLANETS . " AS p
				...
				else
				{
					...
					$attack 			= floor ( ( $fleet_row['fleet_amount'] - $target_data['defense_anti-ballistic_missile'] ) * ( parent::$_combat_caps[503]['attack'] * ( 1 + ( $attacker_data["research_weapons_technology"] / 10 ) ) ) );
					...
					for ( $t = 0 ; $t < count ( $attack_order ) ; $t++ )
					{
						...
						if ( $target_data[parent::$_resource[$n]] )
						{
							$defense		= ( ( parent::$_pricelist[$n]['metal'] + parent::$_pricelist[$n]['crystal'] ) / 10 ) * ( 1 + ( $target_data['research_shielding_technology'] / 10 ) );

							if ( $attack >= ( $defense * $target_data[parent::$_resource[$n]] ) )
							{
								$destroyed	= $target_data[parent::$_resource[$n]];
							}
							else
							{
							...
							if ( $destroyed != 0 )
							{
								...
								$destroyed_query	.= "`" . parent::$_resource[$n] . "` = `" . parent::$_resource[$n] . "` - ".$destroyed . ",";
							}
						}
					}
				...
				$search				= array ( '%1%' , '%2%' , '%3%' );
				$replace			= array ( $fleet_row['fleet_amount'] , $attacker_data['name'] . ' [' . $fleet_row['fleet_start_galaxy'] . ':' . $fleet_row['fleet_start_system'] . ':' . $fleet_row['fleet_start_planet'] .'] ' , $target_data['name'] . ' [' . $fleet_row['fleet_end_galaxy'] . ':' . $fleet_row['fleet_end_system'] . ':' .  $fleet_row['fleet_end_planet'] .'] ' );
				...
				Functions_Lib::send_message ( $target_data['id_owner'] , '' , $fleet_row['fleet_end_time'] , 5 , $this->_lang['sys_mess_tower'] , $this->_lang['gl_missile_attack'] , $message_vorlage . $message );
