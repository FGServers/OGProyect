...
class Transport extends Missions
{
	...
	*/
	...
	{
		$transport_check	= parent::$db->query_fetch ( "SELECT pc1.`id_owner` AS start_id,
																			pc1.`name` AS start_name,
																			pc2.`id_owner` AS target_id,
																			pc2.`name` AS target_name
																	FROM " . PLANETS . " AS pc1, " . PLANETS . " AS pc2
																	WHERE pc1.`galaxy` = '" . $fleet_row['fleet_start_galaxy'] . "' AND
																			pc1.`system` = '" . $fleet_row['fleet_start_system'] . "' AND
																			pc1.`planet` = '" . $fleet_row['fleet_start_planet'] . "' AND
																			pc1.`planet_type` = '" . $fleet_row['fleet_start_type'] . "' AND
																			pc2.`galaxy` = '" . $fleet_row['fleet_end_galaxy'] . "' AND
