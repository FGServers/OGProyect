	...
	public function colonize_mission ( $fleet_row )
	{
		if ( $fleet_row['fleet_mess'] == 0 )
		{
			$colonization_check	= parent::$db->query_fetch	( "SELECT
																	(SELECT COUNT(*)
																		FROM " . PLANETS . " AS pc1
																		WHERE pc1.`id_owner` = '". $fleet_row['fleet_owner'] ."' AND
																				pc1.`planet_type` = '1' AND
																				pc1.`planet_destroyed` = '0') AS planet_count,
																	(SELECT COUNT(*)
																		FROM " . PLANETS . " AS pc2
																		WHERE pc2.`galaxy` = '". $fleet_row['fleet_end_galaxy']."' AND
																				pc2.`system` = '". $fleet_row['fleet_end_system']."' AND
																				pc2.`planet` = '". $fleet_row['fleet_end_planet']." AND
					...
					if ( $this->start_creation ( $fleet_row ) )
					{
						...
						if ( $fleet_row['fleet_amount'] == 1 )
						{
							parent::$db->query ( "UPDATE " . USERS_STATISTICS . " AS us SET
													...
													WHERE us.`user_statistic_user_id` = (SELECT p.id_owner FROM " . PLANETS . " AS p
						...
						else
						{
							...
							parent::$db->query ( "UPDATE " . FLEETS . ", " . USERS_STATISTICS . " SET
													...
													WHERE `fleet_id` = '". $fleet_row["fleet_id"] ."' AND
															`user_statistic_user_id` = (SELECT id_owner FROM " . PLANETS . "
