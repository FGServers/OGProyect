...
class Galaxy_Lib extends OGPCore
{
	const PLANET_TYPE = 1;
	const DEBRIS_TYPE = 2;
	const MOON_TYPE	= 3;
	...
	{
		$phalanx_link	= stripslashes ( $this->_row_data['name'] );
		...
		if ( $this->_row_data['planet_destroyed'] == 0 )
		{
			if ( $this->is_phalanx_active() )
			{
				...
				$phalanx_link	= Functions_Lib::set_url ( '' , 'Phalanx' , $this->_row_data['name'] , $attributes );
			}

			$planetname	=  $phalanx_link;

			if ( $this->_row_data['last_update']  > ( time() - 59 * 60 ) && $this->_row_data['user_id'] != $this->_current_user['user_id'] )
			{
				if ( $this->_row_data['last_update']  > ( time() - 10 * 60 ) && $this->_row_data['user_id'] != $this->_current_user['user_id'] )
				{
					$planetname	.= "(*)";
				}
				else
				{
					$planetname	.= " (" . Format_Lib::pretty_time_hour ( time() - $this->_row_data['last_update'] ) . ")";
				}
			}
		}
		else
	...
	private function moon_block()
	{
		...
		if ( $this->_row_data['destruyed_moon'] == 0 && $this->_row_data['id_luna'] != 0 )
		{
			...
			$parse['system']		= $this->_system;
			$parse['planet']		= $this->_planet;
			$parse['diameter']		= Format_Lib::pretty_number ( $this->_row_data['diameter'] );
			$parse['temperature']	= Format_Lib::pretty_number ( $this->_row_data['temp_min'] );
			$parse['links']			= '';
	...
	private function ally_block()
	{
		...
		if ( $this->_row_data['user_ally_id'] != 0 )
		{
			...
			{
				$parse['tag']		= '<span class="allymember">' . $this->_row_data['ally_tag'] . '</span>';
			}
			else
			{
				$parse['tag']  		= $this->_row_data['ally_tag'];
			}
		}
	...
	{
		if ( ( $this->_current_planet['defense_interplanetary_missile'] != 0 ) && ( $this->_row_data['user_id'] != $this->_current_user['user_id'] ) && ( $this->_row_data['galaxy'] == $this->_current_planet['galaxy'] ) )
		{
	...
	{
		if ( ( $this->_current_planet['building_phalanx'] != 0 ) && ( $this->_row_data['user_id'] != $this->_current_user['user_id'] ) && ( $this->_row_data['galaxy'] == $this->_current_planet['galaxy'] ) )
		{
	...
	{
		$min_system	= $this->_current_planet['system'] - $range;
		$max_system = $this->_current_planet['system'] + $range;
