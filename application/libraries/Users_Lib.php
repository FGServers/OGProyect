...
class Users_Lib extends OGPCore
{
	private $_user_data;
	...
	public function check_session()
	{
		if ( ! $this->is_session_set() )
		{
			Functions_Lib::redirect ( OGP_ROOT );
		}
	}
	...
	private function set_user_data()
	{
		...
		{
			Functions_Lib::message ( $this->_lang['ccs_multiple_users'] , OGP_ROOT , 3 , FALSE , FALSE );
		}
		...
		{
			Functions_Lib::message ( $this->_lang['ccs_other_user'] , OGP_ROOT , 3 ,  FALSE , FALSE );
		}

		if ( sha1 ( $user_row['user_password'] . "-" . SECRETWORD ) != $_SESSION['user_password'] )
		{
			Functions_Lib::message ( $this->_lang['css_different_password'] , OGP_ROOT , 5 ,  FALSE , FALSE );
		}
		...
		parent::$db->query ( "UPDATE " . USERS . " SET
            					...
            					`user_user_lastip` = '". parent::$db->escape_value ( $_SERVER['REMOTE_ADDR'] ) ."',
            					`user_user_agent` = '". parent::$db->escape_value ( $_SERVER['HTTP_USER_AGENT'] ) ."'
		...
		$this->_planet_data	= parent::$db->query_fetch ( "SELECT p.*, b.*, d.*, s.*,
															...
															LEFT JOIN " . PLANETS . " AS m ON m.id = (SELECT mp.id
																										...
																										WHERE (mp.galaxy=p.galaxy AND
																												...
																												mp.planet_type=3))
															WHERE p.`id` = '" . $this->_user_data['user_current_planet'] . "';" );
	}
	...
	{
		$select		= isset ( $_GET['cp'] ) ? intval ( $_GET['cp'] ) : '';
		$restore 	= isset ( $_GET['re'] ) ? intval ( $_GET['re'] ) : '';

		if ( isset ( $select ) && is_numeric ( $select ) && isset ( $restore ) && $restore == 0 && $select != 0 )
		{
			$owned   = parent::$db->query_fetch ( "SELECT `id`
													FROM " . PLANETS . "
													WHERE `id` = '". $select ."'
														AND `id_owner` = '" . $this->_user_data['user_id'] . "';");
