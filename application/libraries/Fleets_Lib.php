...
class Fleets_Lib extends OGPCore
{
	/**
	 ...
	 * return creates the link with the coordinates of the target planet
	 */
	private static function fleet_resources_popup ( $fleet_row , $text , $fleet_type )
	{
	 ...
	 * return creates the link with the coordinates of the target planet
	 */
	private static function fleet_ships_popup ( $fleet_row , $text , $fleet_type , $current_user = '' )
	{
	...
	{
		$enemy_name	= parent::$db->query_fetch ( "SELECT `username`
													FROM " . USERS . "
													WHERE `id` = '" . intval ( $fleet_row['fleet_owner'] ) . "';" );

		$link  		 = $enemy_name['username'] . " ";
	...
	/**
	 * method flying_fleets_admin_table
	 * param
	 * return the fleet table for admin panel
	 */
	public static function flying_fleets_admin_table ()
	{
		$table			= '';
		$FlyingFleets 	= parent::$db->query ( "SELECT *
												FROM " . FLEETS . "
												ORDER BY `fleet_end_time` ASC;");

		while ( $CurrentFleet = parent::$db->fetch_array ( $FlyingFleets ) )
		{
			$owners			  = parent::$db->query_fetch ( "SELECT ( SELECT `username`
																FROM " . USERS . "
																WHERE `id` = '". intval ( $CurrentFleet['fleet_owner'] ) ."') AS fleet_username,
																( SELECT `username`
																	FROM " . USERS . "
																	WHERE `id` = '". intval ( $CurrentFleet['fleet_target_owner'] ) ."') AS target_username" );

			$Bloc['Id']       			= $CurrentFleet['fleet_id'];
			$Bloc['Mission']			= self::fleet_resources_popup ( $CurrentFleet , parent::$lang['type_mission'][$CurrentFleet['fleet_mission']] , '' );
			$Bloc['Mission']   		   .= "<br>" . ( ( $CurrentFleet['fleet_mess'] == 1 ) ? "R" : "A" );
			$Bloc['Fleet']    			= self::fleet_ships_popup ( $CurrentFleet , parent::$lang['tech'][200] , '' );
			$Bloc['St_Owner'] 			= "[" . $CurrentFleet['fleet_owner'] . "]<br>" . $owners['fleet_username'];
			...
			$Bloc['St_Time']  			= date ( Functions_Lib::read_config ( 'date_format_extended' ) , $CurrentFleet['fleet_start_time'] );

			if ( is_array ( $owners ) )
			{
				$Bloc['En_Owner'] 		= "[" . $CurrentFleet['fleet_target_owner'] . "]<br>" . $owners['target_username'];
			}
			else
			{
				$Bloc['En_Owner'] 		= "";
			...
			if ( $CurrentFleet['fleet_mission'] == 5 )
			{
				...
				{
					$Bloc['Wa_Time']  	= date ( Functions_Lib::read_config ( 'date_format_extended' ) , $CurrentFleet['fleet_end_stay'] );
				}
				elseif ( $CurrentFleet['fleet_mess'] == 1 )
				{
					$Bloc['Wa_Time']  	= parent::$lang['cff_back'];
				}
				else
				{
					$Bloc['Wa_Time']	= parent::$lang['cff_to_destination'];
				}
			}
			else
			{
			...
			$Bloc['En_Time']  			= date ( Functions_Lib::read_config ( 'date_format_extended' ) , $CurrentFleet['fleet_end_time'] );

			$table .= parent::$page->parse_template ( parent::$page->get_template ( 'adm/fleet_rows_view' ) , $Bloc );
		}
	...
	public static function flying_fleets_table ( $fleet_row , $Status , $Owner , $Label , $Record , $current_user )
	{
		...
		$planet_name	= parent::$db->query_fetch ( "SELECT	(SELECT `name`
															FROM " . PLANETS . "
															WHERE `galaxy` = '".intval($fleet_row['fleet_start_galaxy'])."' AND
																	`system` = '".intval($fleet_row['fleet_start_system'])."' AND
																	`planet` = '".intval($fleet_row['fleet_start_planet'])."' AND
