...
class Developments_Lib extends OGPCore
{
	/**
	...
	{
		return $current_planet["field_max"] + ( $current_planet[parent::$objects->get_objects(33)] * FIELDS_BY_TERRAFORMER );
	}
	...
	public static function is_development_payable ( $current_user , $current_planet , $element , $incremental = TRUE , $destroy = FALSE )
	{
		...
		{
			if ( $costs[$resource] > $current_planet[$resource] )
			{
				$return = FALSE;
			}
		}
	...
	public static function formated_development_price ( $current_user , $current_planet , $element , $userfactor = TRUE , $level = FALSE )
	{
		...
		foreach ( $array as $res_type => $ResTitle )
		{
			if ( isset ( $pricelist[$element][$res_type] ) && $pricelist[$element][$res_type] != 0 )
			{
				...
				if ( $cost > $current_planet[$res_type] )
				{
					$text .= "<b style=\"color:red;\"> <t title=\"-" . Format_Lib::pretty_number ( $cost - $current_planet[$res_type] ) . "\">";
	...
	public static function set_first_element ( &$current_planet, $current_user )
	{
		...
		if ( $current_planet['planet_b_building'] == 0 )
		{
			...
			if ( $current_queue != 0 )
			{
				...
				while ( $loop == TRUE )
				{
					...
					if ( $is_payable == TRUE )
					{
						...
						$current_planet['metal']       -= $price['metal'];
						$current_planet['crystal']     -= $price['crystal'];
						$current_planet['deuterium']   -= $price['deuterium'];
						$current_time                   = time();
					...
					else
					{
						...
						else
						{
							...
							$message    = sprintf	(
														...
														$element_name,
														Format_Lib::pretty_number ( $current_planet['metal'] ) , $lang['Metal'],
														Format_Lib::pretty_number ( $current_planet['crystal'] ) , $lang['Crystal'],
														Format_Lib::pretty_number ( $current_planet['deuterium'] ) , $lang['Deuterium'],
			...
			parent::$db->query ( "UPDATE " . PLANETS . " SET
									`metal` = '" . $current_planet['metal'] . "',
									`crystal` = '" . $current_planet['crystal'] . "',
									`deuterium` = '" . $current_planet['deuterium'] . "',
									`planet_b_building` = '" . $current_planet['planet_b_building'] . "',
									`planet_b_building_id` = '" . $current_planet['planet_b_building_id'] . "'
									WHERE `id` = '" . $current_planet['id'] . "';" );
	...
	{
		return ( $current_planet['b_hangar'] != 0 );
	}
}
