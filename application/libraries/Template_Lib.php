...
class Template_Lib
{
	...
	*/
	...
	{
		$route		=  OGP_ROOT . TEMPLATE_DIR . $template_name . '.php';
	...
	*/
	...
	{
		$parse['title']			= 'Install';
		$parse['OGP_ROOT']		= OGP_ROOT;
		$parse['js_path']		= OGP_ROOT . JS_PATH;
		$parse['css_path']		= OGP_ROOT . CSS_PATH;
	...
	*/
	private function game_header ( $metatags = '' )
	{
		...
		$parse['-meta-']	.= "<script type=\"text/javascript\" src=\"" . OGP_ROOT . JS_PATH . "overlib-min.js\"></script>\n";
	...
	*/
	private function game_navbar()
	{
		$parse					= $this->_lang;
		$parse['dpath']			= DPATH;
		$parse['image']			= $this->_current_planet['image'];
		...
		// RESOURCES FORMAT
		$metal 		= Format_Lib::pretty_number ( $this->_current_planet["metal"] );
		$crystal 	= Format_Lib::pretty_number ( $this->_current_planet["crystal"] );
		$deuterium	= Format_Lib::pretty_number ( $this->_current_planet["deuterium"] );
		$darkmatter	= Format_Lib::pretty_number ( $this->_current_user["premium_dark_matter"] );
		$energy		= Format_Lib::pretty_number ( $this->_current_planet["energy_max"] + $this->_current_planet["energy_used"] ) . "/" . Format_Lib::pretty_number ( $this->_current_planet["energy_max"] );
		...
		// METAL
		if ( ( $this->_current_planet["metal"] >= $this->_current_planet["metal_max"] ) )
		{
			$metal		= Format_Lib::color_red ( $metal );
		}

		// CRYSTAL
		if ( ( $this->_current_planet["crystal"] >= $this->_current_planet["crystal_max"] ) )
		{
			$crystal	= Format_Lib::color_red ( $crystal );
		}

		// DEUTERIUM
		if ( ( $this->_current_planet["deuterium"] >= $this->_current_planet["deuterium_max"] ) )
		{
			$deuterium	= Format_Lib::color_red ( $deuterium );
		}

		// ENERGY
		if ( ( $this->_current_planet["energy_max"] + $this->_current_planet["energy_used"] ) < 0 )
		{
