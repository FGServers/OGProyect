...
abstract class Functions_Lib extends OGPCore
{
	/**
	 * __construct()
	 */
	...
	public static function load_library ( $library = '' )
	{
		...
		{
			// Require file
			require_once ( OGP_ROOT . 'application/libraries/' . $library . '.php' );
	...
	*/
	...
	/**
	 * method Functions_Lib::unset_vars
	 * param $prefix
	 * return unset variables to clean memory and for security
	*/
	public static function unset_vars ( $prefix )
	{
	...
	*/
	public static function sort_planets ( $current_user )
	{
		...
		$planets  = "SELECT `id`, `name`, `galaxy`, `system`, `planet`, `planet_type`
						FROM " . PLANETS . "
						WHERE `id_owner` = '" . intval ( $current_user['user_id'] ) ."'
	...
	*/
	public static function build_planet_list ( $current_user , $current_planet_id = 0 )
	{
		...
		if ( $user_planets )
		{
			...
			{
				if ( $current_planet_id != $planets['id'] )
				{
					$list .= "\n<option ";
					$list .= ( ( $planets['id'] == $current_user['user_current_planet'] ) ? "selected=\"selected\" " : "" );

					if ( $current_planet_id == 0 ) // FOR TOPNAVIGATION BAR PLANET LIST
					{
						$list .= "value=\"game.php?page=" . $page . "&gid=" . $gid . "&cp=" . $planets['id'] . "";
						$list .= "&amp;mode=" . $mode;
						$list .= "&amp;re=0\">";
					}
					else // FOR FLEETS2 PAGE COLONIES SHORTCUTS
					{
						$list .= "value=\"" . $planets['galaxy'].';'.$planets['system'].';'.$planets['planet'].';'.$planets['planet_type'] . "\">";
					}
					$list .= ( ( $planets['planet_type'] != 3 ) ? "" . $planets['name'] : "" . $planets['name'] . " (" . parent::$lang['fcm_moon'] . ")" );
					$list .= "&nbsp;[" . $planets['galaxy'] . ":";
					$list .= $planets['system'] . ":";
					$list .= $planets['planet'];
					$list .= "]&nbsp;&nbsp;</option>";
	...
	*/
	...
	{
		$langs_dir 		= opendir ( OGP_ROOT . LANG_PATH );
