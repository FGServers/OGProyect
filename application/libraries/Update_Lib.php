...
class Update_Lib extends OGPCore
{
	/**
	 * __construct()
	 */
	...
	{
		include_once ( OGP_ROOT . 'application/libraries/MissionControl_Lib.php' );
	...
	private static function check_building_queue ( &$current_planet , &$current_user )
	{
		...
		if ( $current_planet['planet_b_building_id'] != 0 )
		{
			...
			if ( $build_end_time <= time() )
			{
				...
				$current	= intval ( $current_planet['field_current'] );
				$max		= intval ( $current_planet['field_max'] );
				...
				$current_planet['field_current']			= $current;
				$current_planet['field_max']				= $max;
				...
				parent::$db->query ( "UPDATE " . PLANETS . " AS p
										INNER JOIN " . USERS_STATISTICS . " AS s ON s.user_statistic_user_id = p.id_owner
										INNER JOIN " . BUILDINGS . " AS b ON b.building_planet_id = p.id SET
										...
										`field_current` = '" . $current_planet['field_current'] . "',
										`field_max` = '" . $current_planet['field_max'] . "'
